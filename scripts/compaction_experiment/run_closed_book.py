...
* **It adds the deployed strategy as an arm.** `main` runs the algorithm the released code
  ...
  that checkout with `released_probe.rs`, so "is the new strategy better than what ships" is
...
import harness as base  # noqa: E402  inputs, questionnaire, schedule, Calls, probe
import arms as fid  # noqa: E402  corrected Codex budgeting + AI SDK lowering
import strategies as ext  # noqa: E402
import exam as exam  # noqa: E402
...
def safe_cuts(records, cuts):
    ...
    stored message. `harness.schedule` enforces this for the chunk-based boundaries
    ...
    `arms.safe_plan` applies; applied here so both cadences obey it.
    """
...
# arm and run ordering (see CLOSED_BOOK_PROTOCOL.md).
CACHE_ELIGIBLE = {
...
def main():
    ...
    ap.add_argument("--driver", type=Path, required=True, help="compaction_probe binary")
    ap.add_argument("--main-driver", type=Path, required=True, help="released_probe binary")
    ...
    code_paths = [Path(__file__), HERE / "harness.py", HERE / "arms.py",
                  HERE / "opencode_fidelity.mjs", HERE / "capture_shape.py",
                  HERE / "strategies.py", HERE / "exam.py",
                  HERE / "transport.py"]
    config = {
        "version": 4,
        "seed": SEED,
