...
REPO = HERE.parents[1]
PROBE = Path(os.environ.get("ABC_SHAPE_PROBE", REPO / "target/debug/examples/compaction_probe"))
BASE = Path(os.environ.get("ABC_BASE_PROMPT", Path.home() / "compact-exp/shape/system-prompt.txt"))
EXECUTOR = Path(os.environ.get("ABC_TOOL_EXECUTOR", REPO / "target/debug/examples/production_tool_executor"))
