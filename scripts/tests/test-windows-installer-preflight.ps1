...
try {
    ...
    & $csc /nologo /target:winexe "/out:$fixture" (Join-Path $PSScriptRoot 'windows-installer-fixture.cs')
    if ($LASTEXITCODE -ne 0) { throw 'Fixture compilation failed' }
    $fixtureHash = (Get-FileHash -LiteralPath $fixture).Hash
    $newFixture = Join-Path $root 'fixture-new.exe'
    & $csc /nologo /target:winexe /define:INSTALLER_NEW "/out:$newFixture" (Join-Path $PSScriptRoot 'windows-installer-fixture.cs')
    ...
    & $MakeNsis /V2 /WX /INPUTCHARSET UTF8 "/DTEST_OUTFILE=$installer" "/DTEST_FIXTURE=$newFixture" (Join-Path $PSScriptRoot 'windows-installer-preflight.nsi')
