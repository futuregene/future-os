#!/usr/bin/env python3
"""Offline regression tests: python3 scripts/tests/test-install.py."""
import json
import os
...
INSTALLER = (Path(__file__).resolve().parents[1] / "release" / "install.sh").read_text(encoding="utf-8").rsplit('\ncase "$OS" in', 1)[0]
