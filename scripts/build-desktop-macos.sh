...
usage() {
  cat <<'EOF'
Usage: scripts/build-desktop-macos.sh [options]
...
Examples:
  ...
  scripts/build-desktop-macos.sh --skip-deps --out-dir ./dist
  scripts/build-desktop-macos.sh --identity "Developer ID Application: Example"
  APPLE_NOTARY_PROFILE=futureos scripts/build-desktop-macos.sh
