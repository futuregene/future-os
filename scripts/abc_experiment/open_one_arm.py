...
no report document.
  ...
  python3 open_one_arm.py --closed ~/compact-exp/v4-forced --arm summarized ... --smoke
  # the real thing
  python3 open_one_arm.py --closed ~/compact-exp/v4-forced --arm summarized ...
"""
import argparse
import copy
import json
...
import fidelity_rerun as f  # noqa: E402
...
def smoke(args, identity, records, sid, question):
    ...
    try:
        assert ps.has_guidance(system, args.base_prompt.read_text())
        assert f'"{sid}"' in system, "the guidance must name this archive session"
        names = [t["function"]["name"] for t in tools]
        print(f"  tools               : {names}")
        print(f"  system prompt       : {len(system.encode())} bytes, guidance present")
