...
#
# Usage:
#   scripts/chan-cov.sh                          # whole crate: report + missed list
#   scripts/chan-cov.sh --check FILE [FILE...]   # fail if any listed file has an
...
if [ "$check" = 1 ] && [ "${#files[@]}" = 0 ]; then
  echo "usage: scripts/chan-cov.sh --check FILE [FILE...]" >&2
  exit 2
fi
...
if [ "$check" = 0 ]; then
  python3 scripts/chan-missed.py
  exit 0
fi
