...
.DESCRIPTION
    Replicates the CI "Windows portable" pipeline (.github/workflows/build.yml):
      1. compile the unified `future` CLI (release) and stage it as the Tauri
         sidecar,
      2. build the desktop app with Tauri (--no-bundle: just the .exe, no installer),
    ...
    Windows 10/11). Keep FutureOS.exe and future.exe together — the desktop app starts
...
.EXAMPLE
    pwsh scripts/build/build-desktop-windows-portable.ps1
    pwsh scripts/build/build-desktop-windows-portable.ps1 -SkipDeps -OutDir C:\builds
    pwsh scripts/build/build-desktop-windows-portable.ps1 -Sign
    pwsh scripts/build/build-desktop-windows-portable.ps1 -Sign -CertSubject "<part of the cert Subject>"
#>
[CmdletBinding()]
param(
    [switch]$SkipDeps,
...
# Repo root = grandparent of this script's directory. Run everything from there so
# relative paths match the CI steps.
$Root = Split-Path -Parent (Split-Path -Parent $PSScriptRoot)
Set-Location $Root

# Find-SignTool / Resolve-SigningCert / Invoke-SignFile, shared with
# build-desktop-windows-installer.ps1 and sign-file.ps1.
. "$PSScriptRoot\..\release\lib\windows-signing.ps1"
...
# desktop build.rs) so they all agree. version.mjs derives it from git:
...
Write-Host "==> Building desktop (Tauri, no installer)" -ForegroundColor Cyan
...
# bundling) so the desktop app finds it next to its own exe.
Copy-Item "desktop/src-tauri/target/release/futureos.exe" (Join-Path $stage "FutureOS.exe") -Force
