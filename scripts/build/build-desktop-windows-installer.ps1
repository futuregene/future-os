...
.DESCRIPTION
    ...
    exists separately:
      ...
      * The app .exe, the NSIS plugin DLLs and the setup .exe are signed by the
        Tauri bundler itself, which calls scripts/release/sign-file.ps1 through
...
.EXAMPLE
    pwsh scripts/build/build-desktop-windows-installer.ps1
    pwsh scripts/build/build-desktop-windows-installer.ps1 -Sign
    pwsh scripts/build/build-desktop-windows-installer.ps1 -Sign -SkipDeps
#>
[CmdletBinding()]
param(
    [switch]$SkipDeps,
...
if ($Sign) {
    ...
    $overlay = New-SignOverlayConfig -Thumbprint $signThumbprint `
                                     -SignScript (Join-Path $PSScriptRoot "..\release\sign-file.ps1") `
