...
usage() {
  cat <<'EOF'
Usage: scripts/build/build-desktop-macos.sh [options]

Build the unified `future` CLI and Tauri desktop app, then produce a macOS DMG.
...
Examples:
  scripts/build/build-desktop-macos.sh
  scripts/build/build-desktop-macos.sh --skip-deps --out-dir ./dist
  scripts/build/build-desktop-macos.sh --identity "Developer ID Application: Example"
  APPLE_NOTARY_PROFILE=futureos scripts/build/build-desktop-macos.sh
