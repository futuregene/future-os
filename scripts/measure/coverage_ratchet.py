...
Two modes:
  ...
  emit  <llvm-cov.json> <out.json>
      ...
      scripts/measure/coverage.sh runs this on every measurement; the output
  ...
  check --baseline <floors.json> --current <baseline.json>
      ...
      file (scripts/measure/coverage-baseline.json), the current line-coverage
...
def emit(llvm_cov_json: Path, out: Path) -> int:
    ...
    # scripts/measure/coverage.sh invokes this from the repo root.
    ...
    doc = {
        ...
        "metric_basis": (
            "cargo llvm-cov summary via scripts/measure/coverage.sh; "
...
def check(baseline_path: Path, current_path: Path) -> int:
        ...
        if current_pct + 1e-9 < floor_pct:
            ...
            if current_pct > floor_pct:
                ...
                rises.append(
                    ...
                    "(consider bumping scripts/measure/coverage-baseline.json)"
                )
    ...
    if failures:
        ...
        print(
            ...
            "lower the floor in scripts/measure/coverage-baseline.json in this PR — "
