#!/usr/bin/env bash
# scripts/measure/coverage.sh — workspace test coverage via cargo llvm-cov.
#
# Usage:
#   scripts/measure/coverage.sh                 # full workspace run
#   scripts/measure/coverage.sh --check         # run + enforce the coverage ratchet
#                                       # against scripts/measure/coverage-baseline.json
#                                       # (CI mode; exit 1 on any floor drop)
#   scripts/measure/coverage.sh -p future-tui   # subset (extra args forwarded to
...
#                    (scripts/measure/coverage_ratchet.py emit; P1-7 ratchet input)
...
python3 scripts/measure/coverage_ratchet.py emit "$out_dir/llvm-cov.json" "$out_dir/baseline.json"
cargo llvm-cov report --summary-only

# P1-7 ratchet: line coverage may only go up; an intentional drop is approved
# by lowering the floor in scripts/measure/coverage-baseline.json in the same PR.
if [[ "$check" == 1 ]]; then
  python3 scripts/measure/coverage_ratchet.py check \
    --baseline scripts/measure/coverage-baseline.json \
