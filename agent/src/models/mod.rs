...
pub fn user_models_path() -> String {
    let home = dirs::home_dir()
        .map(|h| h.join(".xihu/models.json"))
        .unwrap_or_else(|| std::path::PathBuf::from("/tmp/.xihu/models.json"));
    home.to_string_lossy().to_string()
}

/// SettingsPath returns ~/.xihu/settings.json.
pub fn settings_path() -> String {
    let home = dirs::home_dir()
        .map(|h| h.join(".xihu/settings.json"))
        .unwrap_or_else(|| std::path::PathBuf::from("/tmp/.xihu/settings.json"));
    home.to_string_lossy().to_string()
}

/// Settings represents the xihu settings.json format.
...
/// LoadSettings reads ~/.xihu/settings.json.
