...
#[test]
fn legacy_checkpoint_boundary_can_rebuild_evidence_from_intact_raw_history() {
    let raw = vec![
        message("u", "user", vec![ContentBlock::text("keep original")]),
        message("a", "assistant", vec![call("x", "config.json")]),
        message("t", "tool", vec![result("x", "version=legacy", false)]),
    ];
    let manager = ContextManager {
        enabled: true,
        reserve_tokens: 1600,
    ...
    legacy.algorithm_version = "semantic-v1".into();
    legacy.protected_entry_ids.clear();
    legacy.summary = vec![ContentBlock::text("OLD_SUMMARY_SENTINEL")];
    let projection = crate::compaction::project_prompt_context(&raw, Some(&legacy), None, 8000);
    let ContextPreparation::Compacted { checkpoint, .. } = prepare(
        &manager,
        projection,
        &raw,
    ...
    let text = checkpoint
        ...
        .collect::<String>();
    assert!(text.contains("version=legacy"));
    assert!(!text.contains("OLD_SUMMARY_SENTINEL"));
    assert_eq!(checkpoint.cutoff_entry_id.as_deref(), Some("t"));
    let restored = crate::compaction::project_prompt_context(&raw, Some(&checkpoint), None, 8000);
    assert!(restored
        .messages
        .iter()
        .any(|m| m.message.journal_entry_id() == Some("u")));
    assert!(restored.messages.iter().all(|m| m
        .message
        .content
        .iter()
