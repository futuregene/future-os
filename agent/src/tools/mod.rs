...
async fn spawn_shell(
    ...
    // Unix: stderr is nulled, nothing to drain.
    ...
    //
    // On Windows the CLI terminates itself via TerminateProcess/process.abort.
    // PowerShell cannot observe this exit and spawned.wait() would hang
    // forever.  The PowerShell wrapper already merges stderr into stdout via
    // its own 2>&1, so EOF on the combined pipe means all output is complete
    // and fd 1 is closed — disarm the Job and return immediately.
    #[cfg(windows)]
    {
