...
pub fn handle_command_internal(state: &AppState, cmd: RpcCommand) -> String {
    ...
    match cmd_type.as_str() {
        "prompt" => return run_control::handle_prompt(state, &session, &cmd, id),
        "cancel_queued_run" => return run_control::handle_cancel_queued_run(&session, &cmd, id),
        "prune_run_events" => return run_control::handle_prune_run_events(&session, &cmd, id),
        "abort_session" => return run_control::handle_abort_session(&session, id),
        "retry_persistence" => return run_control::handle_retry_persistence(&session, id),
        "abort" => return run_control::handle_abort(state, &session, &cmd, id),
        "approval_decision" => return run_control::handle_approval_decision(state, &cmd, id),
        "get_state" => return observability::handle_get_state(state, &cmd, id),
        "get_messages" => return observability::handle_get_messages(&session, id),
        "get_events_since" => return observability::handle_get_events_since(&session, &cmd, id),
        "get_session_events_since" => {
            return observability::handle_get_session_events_since(&session, &cmd, id)
        }
        "set_model" => return settings::handle_set_model(&session, &cmd, id),
        "set_thinking_level" => return settings::handle_set_thinking_level(&session, &cmd, id),
        "compact" => return settings::handle_compact(&session, &cmd, id),
        "set_auto_compaction" => return settings::handle_set_auto_compaction(&session, &cmd, id),
        "set_auto_retry" => return settings::handle_set_auto_retry(&session, &cmd, id),
        "set_system_prompt" => return settings::handle_set_system_prompt(&session, &cmd, id),
        "set_tools" => return settings::handle_set_tools(&session, &cmd, id),
        "disable_tools" => return settings::handle_disable_tools(&session, id),
        "disable_builtin_tools" => return settings::handle_disable_builtin_tools(&session, id),
        "append_system_prompt" => return settings::handle_append_system_prompt(&session, &cmd, id),
        "steer" => return settings::handle_steer(&session, &cmd, id),
        "set_ephemeral" => return settings::handle_set_ephemeral(&session, &cmd, id),
        "shell" => return settings::handle_shell(&session, &cmd, id),
        "get_session_stats" => return observability::handle_get_session_stats(&session, id),
        "get_runtime_metrics" => return observability::handle_get_runtime_metrics(&session, id),
        "fork" => return session_lifecycle::cmd_fork(state, &session, &cmd, id),
        "get_session_entries" => return session_lifecycle::cmd_get_session_entries(&session, id),
        "get_last_assistant_text" => {
            return observability::handle_get_last_assistant_text(&session, id)
        }
        "set_session_name" => return settings::handle_set_session_name(&session, &cmd, id),
        "abort_retry" => return run_control::handle_abort_retry(&session, id),
        "cycle_model" => return settings::handle_cycle_model(state, &session, id),
        "cycle_thinking_level" => return settings::handle_cycle_thinking_level(&session, id),
        "clone" => return session_lifecycle::cmd_clone(state, &session, id),
        "export_html" => return observability::handle_export_html(&session, id),
        "reload_config" => return settings::cmd_reload_config(state, &session, id),
        "set_cwd" => return settings::handle_set_cwd(&session, &cmd, id),
        "add_session_rule" => return settings::handle_add_session_rule(&session, &cmd, id),
        "set_sandbox_policy" => return settings::handle_set_sandbox_policy(&session, &cmd, id),
        "set_permission_level" => return settings::handle_set_permission_level(&session, &cmd, id),
