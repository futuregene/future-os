...
pub fn handle_command_internal(state: &AppState, cmd: RpcCommand) -> String {
    ...
    match cmd_type.as_str() {
        ...
        "suggest_skill" => return providers::cmd_suggest_skill(id, &cmd),
        "probe_sandbox" => return settings::handle_probe_sandbox(id),
    ...
    match cmd_type.as_str() {
        ...
        "set_context_files" => settings::handle_set_context_files(&session, &cmd, id),
        ...
        "set_parent_session" => settings::handle_set_parent_session(state, &session, &cmd, id),
