...
fn build_identity_section(opts: &PromptOptions) -> String {
    ...
    guidelines.push("Write ordinary responses in standard Markdown. To reference a file you created or edited on disk, use [name](futureos://file/<path>) with the exact path from the write tool result. Percent-encode the path (e.g. encode `/` as %2F) so the full path, including a leading slash, survives. This is the correct way to point at files you wrote.".to_string());
    guidelines.push("Only use an id-based reference — [label](futureos://artifact/<id>), [label](futureos://run/<id>), [label](futureos://tool/<id>), [label](futureos://approval/<id>), [label](futureos://review/<id>), or [label](futureos://research/<id>) — when you actually have that object's id from earlier in the conversation or tool results. NEVER invent or guess an id; if you don't have one (e.g. a file you just wrote), use futureos://file/<path> instead. Prefer a reference over pasting long stdout, full diffs, or large file contents inline.".to_string());
