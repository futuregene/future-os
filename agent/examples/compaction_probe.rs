...
//! knows them; capture them from a real turn (see `scripts/compaction_experiment/capture_shape.py`)
...
async fn main() -> Result<()> {
    ...
    // A session's system prompt is its own and is unchanged at every point in the session.
    // It used to grow a recall guidance after a checkpoint, which is why the budget prompt
    // and the outgoing prompt were computed separately: the budget had to reserve the growth
    // in advance so committing a checkpoint could not push the next request past admission.
    // With the guidance gone there is one prompt, and the runtime sends it from the budget
    // computation through to the request.
    let base = base_system_prompt.as_deref().unwrap_or_default();
