...
后续快照优化已完成，见 [快照 + 增量及 A/B 实测](streaming-sync-snapshot-optimization.md)。本报告的受控截止点实验保留原始事件 bootstrap（测量入口显式关闭 `preferSnapshot`），用来复现缓存是否复用；本报告约 12.7 秒的恢复耗时不是新默认路径的耗时。
