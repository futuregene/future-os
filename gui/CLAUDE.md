...
> 开发文档统一放在 `gui/DEV_MD/`，下表路径均相对 `gui/`。

| 文档 | 内容 | 何时查 / 改 |
|---|---|---|
| `DEV_MD/PRODUCT.md` (~21KB) | 产品定位、模块边界、工作对象语义、桌面体验 | 改产品行为/加功能/确认领域语义时**查**；产品决策变了才**改** |
| `DEV_MD/ER.md` (~33KB) | 数据对象与关系、表清单、schema 设计决策 | 改 store/数据流时**查**；schema 变更必须同步**改** |
| `DEV_MD/COLOR.md` (4KB) | 配色语义 token + 用法速查 | 选色/改样式时**查**；加/改 token 才**改** |

> `DEV_MD/PRODUCT.md`/`DEV_MD/ER.md` 较大：用 `Read` 的 `offset/limit` 读下面目录里的**对应章节**，不要整篇载入。
...
> **Shadow Review**（Run 级「上一轮变更」）的产品语义见 DEV_MD/PRODUCT.md §4.7，数据模型见 DEV_MD/ER.md §4.10，设计取舍见 DEV_MD/ER.md §6.8。改动影子仓 / 快照 / changeset 相关代码（`src-tauri/src/shadow_review/`、`store/review_snapshots.rs`）前先读这三处。

> **Provider / 模型 / FutureGene 登录**：产品行为见 DEV_MD/PRODUCT.md §5.6，存储与登录实现见 DEV_MD/ER.md §6.9，字段校验见 DEV_MD/PLAN.md「自定义 Provider 字段校验」。改动 `agent_providers.rs` / `auth_store.rs` / `future_login.rs` / `commands/login.rs` 前先读这几处。
...
1. **颜色**：只用 `DEV_MD/COLOR.md` 的语义 token，不写裸 Tailwind 色（`blue-300`…）。状态徽章用 `<Badge tone>`；分类色（事件类别 / 错误子类型）是有意例外。
