...
已具备：输入框附件不复制到工作目录、不自动登记 Artifact；三种上传方式（附件按钮 / 复制粘贴 / 拖拽）；图片每轮最多 4 张、单张最大 25 MiB，并在发送前完成解码校验；非图片不限类型、大小和数量，以安全 JSON 路径清单交给 Agent 按需读取；历史图片/Markdown 应用内预览，其他类型走系统默认应用。
