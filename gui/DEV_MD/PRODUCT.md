...
- 每一轮最多附加 4 张图片；非图片文件不限制数量与大小。单张图片最大 25 MiB，选择、拖拽和粘贴均执行校验。
- 图片发送前必须成功读取并解码；失败时本轮不发送、保留输入框草稿并明确提示具体文件。
...
输入框支持本地文件附件，三种上传方式：附件按钮、复制粘贴、拖拽文件；每轮最多 4 张图片，非图片不限数量。图片走多模态，其他文件以安全转义的结构化路径清单交给 Agent，详见 4.12 Attachment。
