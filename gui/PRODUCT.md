...
桌面 GUI 应该提供最完整、最细腻的体验，包括对话、workspace、run 状态、工具调用、审批、review、artifacts 和长任务进度。GUI 不应该只是终端包装器，而是把 Agent 的工作过程渲染成可理解、可操作的界面。
...
输入框附件入口应该支持任意文件。普通 Chat 中用户主动附加的文件也应该进入 Artifacts，便于后续查看、转入 Research 或复用。

### 4.4 Run

Run 是一次 Agent 执行，通常由一条用户消息触发。Run 需要记录状态、模型、开始和结束时间、错误信息、工具调用过程和输出结果。

用户应该能够看到当前 run 是否正在执行、是否失败、是否等待审批、是否可以取消、重试或恢复。
...
GUI 负责展示审批请求并把 approve / reject 决策回传给 Agent。

### 4.7 Review

Review 解决“改了什么”。它不同于 Approval。Approval 是执行前允许与否，Review 是执行后查看变更。

Review 需要覆盖代码 diff，也需要覆盖 markdown、文档、表格等文本类 artifact 的变更审查。第一版可以先基于文件级 diff 和摘要展示。

### 4.8 Artifact

Artifact 是 Agent 或用户在工作过程中产生的可复用产物，包括文档、报告、表格、图表、diff、命令输出、文献摘要、实验计划和数据分析结果。

Artifact 在 workspace 场景中可以关联文件系统；在普通 Chat 场景中可以由 FutureOS 自动管理。
...
左侧负责功能入口、workspace 和 thread 导航。中间负责 conversation。右侧负责 context、inspection、runs、approvals、review 和 artifacts。

右侧面板不能抢占用户注意力。只有在新的待审批请求出现时，才应该主动切到 Approvals；普通后台刷新不应该造成闪烁或频繁切 tab。
...
右侧面板用于检查当前 Thread 的运行上下文。当前重点：

- Runs：展示 run 状态、事件和工具活动。
- Approvals：展示待审批操作，并支持 approve / reject。
- Review：展示变更审查入口。
- Artifacts：展示当前 Thread 或 Workspace 的产物。
...
7. 高风险操作进入 Approval。
8. GUI 展示文本增量、工具活动、审批和结束状态。
9. Run 完成后 assistant message、run events、tool calls、tool outputs 和 approval 记录被持久化。
