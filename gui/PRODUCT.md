...
审批的对象是**文件路径访问**（读 / 写），不是命令。设计与实现细节见 [`APPROVAL_PLAN.md`](APPROVAL_PLAN.md)（规则模型、分层、规则文件）与 [`SANDBOX_PLAN.md`](SANDBOX_PLAN.md)（Seatbelt 强制、escalation、降级）。要点：
...
- **网络完全放开、不审批**；bash 命令本身不预审批（在 OS 沙盒里跑，越界失败后走升级审批）。read/write/edit 工具按规则前置弹审批。
- **敏感文件**（`.env`、`*.pem`、`*.key`、`~/.ssh`、凭证等）默认 ask 且**不可被规则覆盖**——只能“允许一次”，不能持久放行；应用自身凭证（`~/.future/agent/auth.json` 等）与规则文件的写更是硬 deny，跳过用户意愿。
- **只有 GUI 会话启用**这套审批；TUI / CLI / channels 全放开（无沙盒、无审批）。GUI 的“自动审批”开关等于关闭审批保护。
...
GUI 负责展示审批请求、把决策回传给 Agent，并在“本工作区允许”时经可信路径写规则文件（agent 工具无权写该文件）。
