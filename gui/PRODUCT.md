...
输入框附件入口支持图片、PDF 和文本文件。图片作为多模态输入传给模型；PDF / 文本会被抽取文本并内联进上下文（同时附带文件路径，便于模型按需读取原文件，如扫描件或超出截断上限的内容）。附件是否落盘到工作目录、是否登记为 Artifact，按对话类型（普通 Chat / Workspace 对话）判断，详见 4.12 Attachment。
...
- 简单只读 shell 命令可自动放行，例如 `pwd`、`ls`、`find`、`rg`、`grep`、`head`、`tail`、`cat`、`wc`、`sed`、`echo`、`future`、`future`。
- 自动放行只适用于简单命令；复杂 shell、重定向、命令替换、链式执行和高风险操作需要审批。
- workspace 外读取**当前自动放行**（原计划纳入审批，尚未实现；见 ER.md §4.8 `outside_workspace_read`）。
...
用户在普通 Chat 中上传的附件（图片 / PDF / 文本）也作为 Artifact 管理：文件会被保存进对应（临时）工作目录，并登记为 Artifact，便于后续查看、转入 Research 或复用。Workspace 对话中上传的附件不保存到工作目录、不创建 Artifact，只按类型传给模型（图片多模态、PDF / 文本内联）。详见 4.12 Attachment。

普通 Chat 的 Artifacts 面板顶部提供**筛选框**和**上传按钮**：上传可选任意文件（大小约束与附件一致），文件被直接复制进（临时）工作目录并登记为 Artifact，供 Agent 后续运行使用。Artifact 卡片只展示标题、时间和内容，不展示自动生成的 summary 与类型徽章。
...
Attachment 是用户在对话输入框中附加、供模型理解的内容。支持图片、PDF 和文本文件三类。

附件来源与上传方式：

- 接受图片（jpg、jpeg、png、gif、webp、bmp、svg）、PDF 和文本 / 代码文件（按扩展名与内容判断，二进制或不支持的类型不进入附件）。
- 支持三种上传方式：点击输入框的附件按钮、复制粘贴、拖拽文件。
- 每一轮对话最多附加 4 个文件，超出时不再继续添加。
- 传给模型的方式按类型区分：图片作为多模态输入（让模型理解图片内容）；PDF / 文本被抽取纯文本后内联进上下文，并附上文件路径（便于模型按需直接读取原文件——例如无可提取文本的扫描件，或超出内联上限的内容）。内联有单文件与总量的大小 / 行数上限，超出即截断。

附件是否落盘到工作目录、是否登记为 Artifact，按对话类型判断：

- 普通 Chat（没有用户选择的 workspace，背后是临时 workspace）：附件（无论类型）保存到临时工作目录并登记为 Artifact，同时按类型传给模型（图片多模态、PDF / 文本内联）。
- Workspace 对话：附件不保存到工作目录、不创建 Artifact，只按类型传给模型，避免污染用户的项目目录。
...
assistant 的**思考过程**（模型 reasoning）按发生顺序**内联**展示在文本与工具调用之间（与工具调用同处一条时间线，不聚集在消息顶部），以暗色弱化的样式呈现。是否展示由设置「通用」页的**显示思考过程**开关控制，**默认关闭**。

输入框应保持底部悬浮，模型选择位于输入框区域。发送消息时 GUI 创建 Run，写入用户消息，连接 Agent stream，并把事件投影到数据库和 UI。

输入框区域除模型、思考等级选择外，还提供**审批模式**（自动审批 / 手动审批）的快速切换下拉；它与设置「通用」页里的自动审批是同一个全局开关。

每条消息下方提供**复制按钮**（用户消息与 assistant 消息都有），复制该消息的纯文本内容；复制按钮平时隐藏，hover 消息行时才显示。assistant 消息**流式生成期间**例外：底部信息栏（用时·token）常驻显示，且复制按钮位置改为常显的**生成中动态指示**（不是复制按钮）；生成结束后才恢复复制按钮及其 hover 显隐逻辑。

输入框支持图片、PDF 和文本附件，三种上传方式：附件按钮、复制粘贴、拖拽文件；每一轮对话最多附加 4 个文件。附件如何传给模型（图片多模态、PDF / 文本内联）、是否落盘按对话类型判断，详见 4.12 Attachment。
...
- **通用**：界面语言切换（中文 / English，默认中文，选择存于本地）；桌面级开关（自动审批、显示思考过程，均默认关闭）。
- **提供商**：
