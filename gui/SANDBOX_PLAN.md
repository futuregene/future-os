...
### Phase 1 — macOS 沙盒执行 + 最小升级流（核心价值；**不含规则引擎**）
...
- [ ] `agent/src/sandbox/mod.rs`：`SandboxSettings`、平台探测（macOS `sandbox-exec` 存在性）、Seatbelt profile 模板与参数注入、命令包裹（trait 化，为未来替换实现留位）。
- [ ] 路径判定规范化（§3.5）：`~` 展开到真实 HOME、最近存在祖先 canonicalize、symlink 最终路径、macOS 大小写不敏感；`path_is_outside_workspace` 改造 + 单测。
- [ ] `run_bash` 接入：按 scope 的 sandbox 设置包裹执行；full-access / 降级时保持现状。
- [ ] proto `RpcCommand.sandbox_policy` typed 字段 + `set_sandbox_policy` 命令 + `ServerSession` 存储 + `ToolExecutionScope` 透传（本阶段 GUI 固定下发 `workspace-write × on-request × 空规则`，无 UI）。
- [ ] 沙盒内自动放行：`approval_shape` 对"将在沙盒内执行的 bash"返回 None（不产生审批），`tool.sandboxed` run event。
- [ ] **后置审批承载点**：`EscalationRequester` 回调（rpc 层构造、经 `ToolExecutionScope` 注入，见 §2.6）；bash 工具 `escalated/justification` 参数 + 失败特征启发式 → `sandbox_escalation` 审批 → 允许后该次命令出沙盒重跑。
- [ ] `sandbox_boundary` payload 真实化；GUI ApprovalPrompt 识别新 kind（最小渲染）。
- [ ] write/edit 边界判定改 `writable_roots` 集合（复用 §3.5 规范化）。
- [ ] **Profile smoke tests**（macOS，`#[ignore]` 标记、本机手跑 + 后续 CI）：`cargo build`、`npm test`、`git status/commit`、`python -c`、写 `$TMPDIR` 与 `/tmp`、读 `~/.ssh` 被拒、`network_access=false` 时 `curl` 失败、`/dev/stdout`/`/dev/tty` 可用。以此校准 `mach-lookup`/`sysctl` 与伪设备清单。
- 验收：macOS 上 `workspace-write` 会话中 `cargo build` / `npm test` / `git status` 全程无审批卡片；`curl example.com` 失败并弹出升级审批，允许后成功；`~/.ssh` 读被拒；workspace 内指向外部的 symlink 写入被正确判为越界；Windows/Linux 行为与今天一致；`make test` + 新增 sandbox 单测（profile 生成、路径规范化、特征匹配）+ smoke tests 通过。
