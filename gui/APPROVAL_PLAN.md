...
- 未知字段忽略（向前兼容）。**预留规则类型**：`{ "domain": "github.com", "action": "..." }` 网络域名规则——本版不实现、解析时跳过并告警；将来若需域名级控制，经本地代理层实现（见 SANDBOX_PLAN §网络），schema 不用改。
...
| **bash**（子进程，无法预知会碰哪些文件，Seatbelt 只有二态且不能中途问人） | `ask` 与 `deny` 都编译进 Seatbelt profile 当"拒"；命令直接跑，撞上被拒文件 → 失败 → **escalation 审批**（"命令疑似被沙盒拦截，是否放行重跑"）。批准 = 该命令出沙盒跑一次（详见 SANDBOX_PLAN §escalation）。 |

推论：bash 不再有任何**前置**审批——v1 的 `is_workspace_read_command` 白名单、`command_prefix` 规则、`untrusted/on-request/never` 三档策略在启用新模型的会话里全部退役（非启用会话保留现状，见 §7）。
...
| 客户端 | 规则系统 | 行为 |
|---|---|---|
| **GUI** | 启用（session 建立时经 `set_sandbox_policy { enabled: true }` 下发） | 本文全部语义 + bash Seatbelt 强制 |
| TUI / CLI / channels | 不启用 | 与现状一致（既有 `permission_level` 语义），规则文件与沙盒不参与 |
| GUI「自动审批」开关 | 开 = `enabled: false` | 完全放开：无规则、无沙盒、无审批（即 v1 的 danger-full-access；UI 标红） |

v1 的 `read-only / workspace-write / danger-full-access` 三种模式与 `untrusted / on-request / never` 三档策略**全部收敛为这一个布尔**：审批保护开/关。这是"配置简单易懂"的直接体现。
...
- **无命令级护栏**：`git push --force`、`npm publish`、workspace 内 `rm -rf .` 都不问（Shadow Review 事后可见）。命令级 allow 在本模型中是空操作（bash 无前置审批可跳过）、deny 可被轻易绕过给假安全感，故整体不做；若试用后发现确需"外向命令减速带"，再评估内置 command-ask 名单。
- escalation 批准 = 整条命令出沙盒（含其一切文件访问），非精确放宽。
- Linux / Windows 无 OS 强制：工具层三态照常生效，bash 层无 Seatbelt（详见 SANDBOX_PLAN §降级）。
...
| V2 | 网络访问完全放开、不审批；schema 预留 domain 规则类型（不实现），将来经本地代理层补域名控制 |
...
| V8 | 三模式 × 三策略收敛为单布尔 enabled；GUI"自动审批"开 = enabled:false |
| V9 | 接受"网络放开 + 清单外密文可泄"与"无命令级护栏"两项残留风险 |
