...
| 字段 | 默认值 | 含义 |
|---|---|---|
| `grpc_addr` | `auto` | 每用户本地 IPC。显式 `http://host:port` 具有权威性——连接失败会直接报错，不会重定向到本地 IPC；Agent 需通过 `--grpc-addr` 开启 TCP。 |
...
| `domain` | `feishu` | API 域名。接受三种形式：`"feishu"` → `open.feishu.cn`、`"lark"` → `open.larksuite.com`（api/ws 基址），或完整 `http(s)://` URL 原样使用（自建网关 / 测试 mock）。 |
