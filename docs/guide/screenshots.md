...
| A multi-touch scene only proves the gesture *reaches* the component | In the browser `measureInWindow` reports **page** coordinates while the dispatched TouchEvents use **viewport** coordinates; on a scrolled page the two differ by a fixed offset, so a zoom's **focal offset in a capture is not trustworthy**. Assert on "the scale changed", never on "this point stayed put"; focal anchoring is covered by unit tests (`mobile/src/features/chat/__tests__/zoomableImage.test.ts`). |
