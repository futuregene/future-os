...
|---|---|---|
| `grpc_addr` | `auto` | Per-user local IPC. An explicit `http://host:port` is authoritative — connection failures are reported, not redirected to local IPC; the agent must opt into TCP with `--grpc-addr`. |
...
| `domain` | `feishu` | API domain. Three accepted forms: `"feishu"` → `open.feishu.cn`, `"lark"` → `open.larksuite.com` (api/ws base URLs), or a full `http(s)://` URL used verbatim (self-hosted gateways / test mocks). |
