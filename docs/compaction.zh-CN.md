...
运行时默认是 C3，算法标识 `c3-sticky-summary-v1`：C 的投影，外加一份写在证据索引之后、由模型生成的交接摘要。摘要是“粘性”的——它会收到上一轮摘要——因此事实在连续压缩中累积，而不是每次被重写。确定性 C（`deterministic-s2-evidence-v1`，schema 3）仍作为兜底：无 provider 可用或摘要调用失败时提交它，此时 `summary` 字段只存放证据索引。保护原文按 `protected_entry_ids` 从 journal 恢复；fork 重映射引用，非法范围／引用会被拒绝。

**摘要贡献了什么，是实测而非假定。** [实验](compaction-abc-experiment.zh-CN.md)中的每一份考卷都没能找到收益：它没有增加投影尚未包含的值，闭卷得分有无摘要完全相同，在压缩压力下至多保住 1 个确定性路径丢弃的 assistant 块。唯一被测出的效果是开卷上一处小幅且统计不显著的差异。这界定了它在这些考卷上的价值，但**不足以证明它在生产环境中无用**——它本应帮助的问题，并非这些考卷所问。

## 摘要读取什么，以及为什么这是便宜的形状

摘要请求把活动对话作为**真实消息数组**发送，指令**追加在最后**，并复用 agent 自己的 system prompt。指令追加而非前置、材料不拍平成单条用户消息，原因在于 provider 按**请求前缀**做缓存：复用会话已经发送过的回合的请求可以命中缓存，而拍平的请求没有任何共享前缀、每次都按全量计费。在真实 provider 上实测，消息数组形状在 25.8 万 tokens 前缀上命中 **99.9%**，费用约为同样输入冷启动的 **1/48**。读取活动对话同时让摘要描述真实历史，而不是描述它那份已经有损的索引。

为摘要预留的预算最多是证据预算可让出部分的三分之一：预算紧时宁可不生成摘要，也不让证据索引变得不可用。
...
默认手动、自动和 provider 超限恢复均使用 C。确定性 C 路径**摘要模型调用数为零**；C3 默认每次压缩会追加一次摘要请求，按普通请求计费，该请求在缓存命中时成本极低（见[实验](compaction-abc-experiment.zh-CN.md)）。使用 C 不重置已累计的正常调用费用。后续正常模型请求仍需支付包含证据索引和查询结果的输入／输出费用；本地扫描也不是零成本。
