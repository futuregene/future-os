...
> [`loop/ARCHITECTURE.zh-CN.md`](loop/ARCHITECTURE.zh-CN.md)，
> 此处不再重复；本页只讲运维模型。
...
- 安装与构建：[build-and-install.zh-CN.md](../guide/build-and-install.zh-CN.md)
- 证据账本：[long-run-evidence-ledger.zh-CN.md](long-run-evidence-ledger.zh-CN.md)
- TUI 使用：[tui.zh-CN.md](../guide/tui.zh-CN.md)
