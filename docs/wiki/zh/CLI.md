...
FutureOS 附带一个**可选的**命令行工具,叫 `future-cli`。它随每个下载包一起附带。
...
> 在 Windows 上,命令行工具在**便携包**里。普通安装版只含应用和它的后台服务,不含单独的 `future-cli.exe`。
...
```bash
future-cli auth login     # 通过浏览器登录
future-cli auth status    # 查看是否已登录
future-cli auth logout    # 登出
```
...
```bash
future-cli run --model sonnet:high "审查这些改动"
future-cli run @README.md "总结这个文件"
echo "一些文本" | future-cli run "把这段文本整理一下"
```
...
- **提示「Connection refused」?** 说明 agent 没运行。执行 `future-cli agent start`,或直接打开桌面应用。
