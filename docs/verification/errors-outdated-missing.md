# 文档核验结果：错误 / 过时 / 缺失清单（历史记录）

> **历史快照，不能用于跳过当前核验。** 每节“已修复/无需修改/PASS”只对原日期和提交有效；
> 旧的 Linux 无沙箱、默认 TCP、每次工具审批等结论已不适用于当前源码。2026-09-07 的现行
> 文档已同步这些变化，见 [文档索引](../README.zh-CN.md)、[审批与沙箱](../wiki/zh/Sandbox.md)
> 和 [手机远程](../wiki/zh/Remote.md)。下面保留原验收过程、行号与结果，不改写历史 PASS。
