...
> **历史快照，不是当前事实清单。** 下文的文件清单、行号、平台范围、默认值和 PASS 只对应
> 原日期/提交；“无需修改/可跳过”不适用于后续版本。2026-09-07 已在现行文档修正审批默认值、
> Linux 沙箱与发布平台、IPC、RPC 双写及技能/CLI 等漂移。保留下文原记录以便追溯；
> 当前入口见 [文档索引](../README.zh-CN.md)，新修改必须重新对照源码。
