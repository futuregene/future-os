...
| 文档 | 内容 |
|---|---|
| [CLI 授权与 Loop 工具化改造](architecture/CLI_REFACTOR.zh-CN.md)（仅中文，已搁置） | 保留 CLI/Loop 与 IPC 隔离调查和旧设计，不再作为远程执行前置 |
| [远程执行方案](architecture/REMOTE_EXECUTION_DESIGN.md)（中文） | SSH Linux runner、执行主机独立技能与 `/` 菜单、目标端上下文与审批规则、桌面面板及后续 WSL2 |
