...
`main` is not a reimplementation: [`released_probe.rs`](../../../scripts/compaction_experiment/released_probe.rs)
...
recorded in [provenance.json](../../../scripts/compaction_experiment/provenance.json).
...
| system prompt | a real production prompt, captured by [`capture_shape.py`](../../../scripts/compaction_experiment/capture_shape.py) |
...
Everything this run needs is in
[scripts/compaction_experiment/](../../../scripts/compaction_experiment/), whose
[README.md](../../../scripts/compaction_experiment/README.md) has the commands; the design
rationale is in [CLOSED_BOOK_PROTOCOL.md](../../../scripts/compaction_experiment/CLOSED_BOOK_PROTOCOL.md).

In outline:

```sh
# build the drivers (in this checkout, and in the checkout being measured)
...
`--force-compaction` above is the fair mode; drop it for the as-deployed table, and give that
run its own `--output`. `--report-only` re-derives `report.json` from an existing run without
calling a model, so the tables can be rebuilt for free.

Inputs, ledgers and results live outside any repository, because they include real session
data: the synthetic chains can be regenerated byte-identically from seeded generators
(`freeze_sessions.py`), and the real-session chains cannot be published at all. Scripts stop
with an error when a required input is missing rather than skipping it, so a partial run cannot
be mistaken for a complete one. Third-party reproduction of the real-session half requires
substituting your own sessions; the absolute numbers will differ and the comparisons should
hold.

Before quoting any number: run `verify_request_shape.py`, which needs no model and checks that
the exam is still sending production's prompt and tools.
