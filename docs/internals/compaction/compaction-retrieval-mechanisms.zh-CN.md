# C/C3、Codex、OpenCode：历史检索机制与实际执行链

## 结论与版本边界

**C 与 C3 共用同一套原生历史检索机制。** 差别在压缩后的投影是否附带粘性模型摘要，不在搜索后端。不能根据之前开卷分数说“C3 没有原生查询”或“只能查摘要”。
...
```text
shlex.split(整条命令) → 一次 future 进程
```

导致 `--json;`、`--limit 5;` 被误当成参数，而不是 shell 命令边界。C3 的 13 次非零 CLI 退出均发生在复合命令中。与此同时，Codex 使用完整原生 shell，查询工作量并不对等。
...
新适配器的工具定义也是原生 Rust `shell_tool().def`，不再手写一个只含 command 的假 shell schema。当前会话的 `future` launcher 只检查**shell 已经解析好的单次 CLI argv**，然后转发给原版 CLI；不解析 shell 源码，所以不破坏循环和管道。该 launcher 是实验会话范围护栏，不是搜索实现。

已经用本地原生程序验证：分号批量搜索、jq 管道、for 循环、search→get 字节定位、原生非零退出、异会话拒绝、指定路径读取拒绝和工作区写入拒绝。没有发起模型考试。

**安全与实验状态不能夸大：**新适配器沿用 Future 原生路径规则，加显式拒绝项；它不宣称是全文件系统读取白名单。Future 当前原生沙箱网络也并非默认禁用。恢复完整 shell 后需要重新审查实验数据隔离，与对照方对齐；新考试入口要求额外的操作员确认，尚未启动付费重测。修好调用链不等于所有公平性验收已完成。
