...
经济触发线为 `floor(W × 0.8)`——**声明窗口的 80%，没有绝对上限**。`effective_trigger` 会把它夹到 `W − O − margin`，其中 `O` 是模型声明的输出上限、`margin = min(2048, W/16)`，因此输出预留很大的模型由自身限制而不是固定数字决定：1M 窗口、声明 384000 输出上限时触发于 **613952**；同样窗口、声明 16 384 时触发于 800000。

每次模型调用前都检查，包括工具回合后和模型窗口下调时，并预留正常最大输出 `O` 与余量——输入必须装进 `W − O − margin`。估算覆盖 system 文本、工具定义、消息框架与图片／reasoning 的保守成本，并参考上游报告的用量。切换模型本身不触发整理：那次真实请求才检查自己的限制。没有可处理旧历史时，装得下的输入不会仅因越过触发线而被截断；必要内容装不下则明确失败，而不是被静默截断。
