...
- 输入区保留两排：文本输入一排，模型、思考模式、技能 `/`、附件和发送/停止按钮共用一排。
  小于 380pt 或大字号时工具栏收紧间距，模型与思考入口仍同时可见，操作按钮保持 44pt 触控区域。
...
- 输入区仍为文本与工具栏两排；窄屏仅收紧工具栏，不压缩操作按钮热区。
