...
- 工作区本身也可置顶（工作区列表的分组菜单）。置顶的工作区分组排在置顶会话之后、
  所有未置顶工作区之前；标记存储在桌面端，重新配对后仍然保留，桌面端侧栏也按它排序
  （同一列、同一规则）。
  菜单要求桌面端声明 `workspace_pinning_v1` 能力，连接旧桌面端时不显示该操作。
  置顶回包与推送、读取共用版本化工作区快照，迟到回包不会覆盖较新的置顶状态。
- 历史分页加载、按 `(runId, idx)` 去重，并在连接恢复后通过
  `get_events_since` 回补实时事件。
- 支持从系统相册选择或调用系统相机拍摄图片、通过系统文件选择器添加文件附件；
  可下载会话附件并在手机预览图片、Markdown、文本（含代码/配置文件）和 JSON，其余受支持类型交给
...
- 附件来源和文件打开/保存/分享菜单复用 `ActionMenu`；选择来源后等待菜单关闭，再调用系统
  相机、照片或文件选择器。Android 13+ 相册走 `PickVisualMedia`（系统照片选择器）；API 33
  以下如果缺少 Play 服务的照片选择器回退，AndroidX 会把这个契约解析成
  `ACTION_OPEN_DOCUMENT`——那是文件选择器，不是相册，无 Play 服务的华为手机点「相册」
  弹出的就是文件浏览器。这类设备改用 `ACTION_PICK` + MediaStore 图片集合打开图库，
  只有没有图库应用响应时才回退到契约。两条路都不需要整库权限：只授权被选中的照片。
  系统权限窗和系统选择器仍由 OS 绘制。
- 会话返回列表时保留列表实例和滚动位置，不再重新播放列表的反向进入动画。
  ...
  工作区列表与对话列表是同一横向滑动的两页：从工作区左滑进入对话、从对话右滑回到工作区。
  方向不够水平的拖动仍归列表滚动；搜索或多选状态下（此时标签栏隐藏）不响应滑动，
  避免在看不见的情况下切换模式；标签本身仍可点击切换。
...
图片、Markdown、文本（代码/配置文件按纯文本读取）和 JSON 可在应用内预览，其他受支持类型使用系统打开/保存/分享
