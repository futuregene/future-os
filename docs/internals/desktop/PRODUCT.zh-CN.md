...
Workspace 下可以有多个子对话。删除某个 workspace 对话会连同其子对话一起删除（删除是递归的），但不删除 workspace 目录和目录内文件。
...
常规文件审批以**文件路径访问**（读 / 写）为对象；手动模式的 shell 审批以及 macOS / Linux 的单次脱沙盒审批以**整条命令**为对象，不能将卡片列出的路径理解成命令的完整权限边界。共享规则、协议与审批见 [SANDBOX/COMMON.zh-CN.md](SANDBOX/COMMON.zh-CN.md)；平台实现、差异与验收分别见 [macOS](SANDBOX/MACOS.zh-CN.md)、[Linux](SANDBOX/LINUX.zh-CN.md)、[Windows](SANDBOX/WINDOWS.zh-CN.md)。要点：
...
- **审批分三档**（输入框下拉 / 设置页切换，全局生效）：
  ...
  - **沙箱保护**（Linux 已合入，发布前真机矩阵仍需候选版本证据）：shell 在 system Bubblewrap 中运行，网络开放；启动时已存在的精确保护目标和 glob 匹配由 OS 沙箱保护。缺失精确目标及命令中新 glob 匹配目前仅结束后检测，不能阻止可写域内创建，也不构成完整访问审计；缺失 Deny 目标不承诺硬保护，属于下述已接受限制。完整 probe 失败显式回退手动审批，绝不裸跑。安装与限制见 [`SANDBOX/LINUX.zh-CN.md`](SANDBOX/LINUX.zh-CN.md)。
...
- 审批按场景提供三个语义：**不允许 / 仅允许这一次 / 此项目以后都允许**（具体文案可继续调整）。“此项目以后都允许”把卡片表达的同一行为和目标保存为该 workspace 的 allow 规则并即时对本轮生效。敏感文件、无法生成可信目标/规则的请求不提供此选项；macOS/Linux 整命令 escalation 仅在失败诊断能解析出非敏感被阻挡路径时提供，且写入的是该路径的写规则，而不是“以后总在沙箱外执行此命令”。
...
**上线与引导**：Skills 已作为左侧导航的独立入口上线。新用户首次进入会在新对话页看到「5 分钟技能上手」横幅（「技能教学」按钮会自动发起一次带入门指引的对话；右上角 ×（aria「收起教学入口」）收起该横幅）；左侧导航的 Skills 入口显示已安装数量角标与一次性引导气泡（「已为你安装 N 个常用技能」/「你还没有安装技能」，点「知道了」关闭），Skills 页提供「技能怎么用」引导（「教我使用技能」发起同一条教练对话，「使用手册」打开平台手册），已安装技能行的「试试」会在新对话输入框预填 `/技能名`。
...
- 支持三种上传方式：点击输入框的附件按钮、复制粘贴、拖拽文件。网页粘贴板提供 `file://` URI 时，先解码为本机正常路径（含 Windows 盘符、UNC 和 Linux 路径）；否则读取系统资源管理器粘贴板，并统一解析 macOS Finder 的文件引用 URL、Windows Explorer 的 `CF_HDROP`、Linux 文件管理器的 URI 列表。解析到真实绝对路径后，均由后端检查受保护路径、存在性、目录与可读性；通过后只记录该路径，不复制文件。
...
普通 Chat 与 Workspace 对话采用相同的文件策略：不复制到工作目录，消息元数据记录原始绝对路径。图片额外生成持久缩略图；粘贴板只有无路径的 `File`/Blob（例如浏览器或远程来源）时，FutureOS 才复制到私有临时目录，单个普通文件最大 10 MiB、每次最多 10 个且合计最大 20 MiB，发送时再持久化到 thread 的 `images/<threadId>/origin`。无路径图片沿用图片粘贴流程与 25 MiB 单张限制。
...
20 MiB、最多 10 个附件，图片计入其中——一条消息可以放满 10 张图片，后端接受同样的数量），
图片最长边超过 1600px 时在手机端降采样到 1600px，
...
左侧负责功能入口、workspace 和 thread 导航。中间负责 conversation 和即时审批。右侧负责后台程序、review 等可检查上下文。左栏右边缘支持拖拽调整宽度，也支持聚焦分隔条后用左右方向键调整；宽度跨重启保留，窗口缩小时自动限制以留出对话区和已打开的右栏。
...
- New Chat。
- Models（直接跳到「模型」页，见 5.7）。
- Skills（已上线为独立导航入口，带已安装数量角标与首次引导，见 4.8）。
- 手机控制（手机遥控，仅登录 FutureOS 后显示入口，见 5.3）。
- 置顶分区（所有置顶对话）。
- Workspace 列表（分区标题：收起开关 + 「新建工作区」按钮；每个工作区分组另有自己的展开/收起 chevron）。
- Workspace 下子对话（行上带 + / − 树节点开关）。
- Chat 列表（分区标题：收起开关 + 批量选择菜单）。
- Settings（底部账户页脚；登录后位于账户菜单内）。
- 各列表分区与工作区分组均可展开/收起；侧栏只列出 active 对话，归档对话不出现在这里（见 4.2）。

置顶是**全局**的：所有被置顶的对话——无论属于某个 workspace 还是普通 Chat——都集中显示在顶部「置顶」分区；取消置顶后回到各自所属的分组。工作区**分组**也可以置顶（分组菜单）：置顶分组排在工作区列表最前，未置顶的分组在其后保持存储层的最近顺序，标题上显示置顶标记。标记存在工作区上（`workspaces.pinned`），因此手机工作区标签页与桌面侧栏对同一批分组给出同样排序。

对话按 Agent 的父子会话关系展示为最多三层的树（不含 Workspace 标题层），默认收起；点击标题仍直接打开对话。分叉和 loop 派生执行会话都使用同一关系。子对话随根对话分组，即使其工作目录不同；置顶子对话独立进入置顶区并保留自己的子树。缺失、归档或删除的父对话不隐藏存活子对话；更深的历史层级平铺到第三层，不删除或改写 Agent 关系。删除对话是递归的：其子对话一并删除（包括置顶子对话——置顶只是侧边栏位置，不代表谱系）。批量全选包含当前分组中已收起的子对话。置顶对话属于置顶区而非任何分组，因此不会进入批量选择：它不显示复选框，工作区分组与对话区的全选在桌面端和手机端都会跳过它。

子对话的展开开关必须是 **+ / − 树节点开关**（收起 `+`、展开 `−`），与工作区 / 分区标题的折叠 **chevron** 在图标上就是两回事；同列时（移动端两个控件同在 8–24 列）仅靠图标区分，不得用同一图标，也不得用与全站细描边语言不一致的实心图形。

**列规则**：一行 = `[开关列 16px][间距 4px][标题]`，**子行的行首 = 父行的标题列** ⇒ 父标题与其子标题同列，逐层递推；叶子行没有开关列，标题就在行首。
桌面行首：对话（含置顶）16、工作区 28（均保持不变），每层 +20；分组头 = `[chevron 8][4][folder 28][4][名称 48]` —— folder 占掉开关列，名称 48 与第一层会话标题同列（见 ActivityRail）。
移动端行首 8、每层 +20；开关布局宽 16（不用 44），44×44 触控区由 `hitSlop` 补齐（左 16 落进列表内边距、右 4 止于标题列）。

标题空间优先：无子对话的行不预留展开箭头占位，子级行靠行首缩进对齐父标题（不再使用占位空白）。同时普通对话和置顶区（非工作区）的标题与高亮左缘之间保留 16px 起始留白，标题不贴边；其他会话新增或展开子对话不改变无关顶层行的标题起点。

对话行**整行可点击**进入该对话；行末的操作菜单（重命名 / 置顶 / 删除）不会误触发进入。工作区分组标题旁提供「打开工作区」入口，以及分组标题的操作菜单（重命名 / 置顶 / 打开文件夹 / 选择聊天 / 删除工作区）；不提供「新建工作区」入口。

对话右侧的**未读指示**：后台运行结束后显示未读圆点（正常完成为绿色、失败为红色）；运行中显示蓝色等待图标；用户打开或离开该对话后未读圆点清除，空闲状态不显示任何圆点。未读状态存于当前会话（sessionStorage），应用重启后清空。

### 5.3 手机遥控

手机是 Desktop 的远程查看与控制端，工具在电脑本机执行。远程能力必须可见、可控：GUI 模式关闭 Desktop 即断开远程；独立 `futureos-headless` 程序不创建窗口，在终端前台运行，按需展示平台登录与手机配对的二维码及链接，Ctrl+C 关闭入口。无头模式不是静默隐藏窗口或自动后台保活。两种模式下，Desktop 拥有的 Agent 和进行中的对话都随 Desktop 退出而结束；已有的外部 Agent 不由它终止。

本功能的完整产品背景、交互规则、网络接线、配对与权限、架构方案、自恢复与同步契约、支持码及开发计划统一维护在 [远程连接设计](CONNECTION.zh-CN.md)，本节不重复维护。
...
assistant 的**思考过程**（模型 reasoning）与工具调用按发生顺序**内联**展示，不聚集在消息顶部。与移动端一致，连续至少两条已结束的步骤默认聚合为靠右的淡灰摘要（工具图标 ×N · 思考图标 ×N）；正文、压缩标记、运行中的工具以及流式末尾步骤打断聚合。失败项可折叠，但摘要保留警告图标和无障碍失败计数。计数按投影后的步骤条数计算，同类工具组保留自己的内部数量。

点击摘要后按原顺序展开步骤，摘要保持靠右，子步骤回到左侧阅读区域；每条思考或工具均可独立展开正文或命令／路径详情。思考默认折叠，**始终可以点击展开**，不再有全局「显示思考过程」开关。模型正在思考但尚未产生内联思考段时，底部信息栏显示「正在思考中…」提示；思考段出现或运行结束后，不再重复提示。

输入框应保持底部悬浮，模型选择位于输入框区域。发送消息时 GUI 创建 Run 记录并把 prompt 交给 Agent；用户消息与事件由 Agent SQLite 唯一持久化，GUI 观察者把事件流实时投影到 UI 与附加存储（审批等）。Desktop 保留独立的工作区、Thread、Run 展示与审批库，不保存第二份对话正文。

历史展示与持久化规则：

- 首次打开只读最近 10 个用户轮次，上翻再取旧页；工具面板独立按需查询，不拖累正文初次加载。单轮本身特别大时仍按完整轮次返回。
- 新对话仍在首次发送时绑定 Agent session。`null → sessionId` 是绑定完成，不是切换对话：后台校准不清空消息、不禁用输入框。已有画面与缓存刷新失败时保留，提供错误和重试。
- 同一 Thread 从已有 session A 更换为 B 时，明确提示上下文更换；新历史就绪前保留旧画面并阻止发送，成功后整体切换，不混合两个 session。不同 Thread 维持独立视图状态。
- 旧页加载使用阅读锚点保持位置，图片等后续布局变化由同一视口逻辑校正；乱序请求和失效会话的回调不得覆盖当前历史。
- 首次升级自动导入旧 JSONL，原文件保留但不再更新。坏会话整体跳过并报告，其他会话可用；全局数据库故障不能伪装成空对话或回退旧文件。未提交流式增量在异常退出时可能丢失，已提交历史是恢复依据。排队 prompt 仅在内存，重启不恢复队列。

回复结束必须向用户交代，沿用消息内状态分割线，不用整页弹窗或空白回复掩盖错误：主动停止、输出长度限制及压缩流程维持各自提示；异常按有证据的模型、网络/上游连接、超时或软件/后台服务原因展示，并保留已生成内容。缺少正常结束信号但没有足够归因证据时显示“原因未确认”，不能一律归咎模型或网络。各协议认可的正常结束、拒答、内容过滤、暂停分别处理，不要求所有模型发送同一种结束标记。具体判定见 [回复结果语义](../../architecture/response-outcomes.zh-CN.md)。
...
每条消息下方提供**复制按钮**，复制消息的纯文本内容。用户消息的复制按钮在 hover 时显示；assistant 回答结束后，复制按钮与**用时·输出 token**靠右常驻显示。assistant **流式生成期间**，琥珀色的**生成中动态圆点**替代复制按钮，与实时计时器一起显示；模型正在思考但尚未产生内联思考段时，还会显示**「正在思考中…」**提示。
...
- **通用**：界面语言切换（中文 / English，默认中文，选择存于本地）；**审批模式**按平台显示（macOS：手动审批 / 沙箱保护 / 完全放开；Windows host probe 通过时：手动审批 / 写保护 / 完全放开；Linux Bubblewrap host probe 通过时：手动审批 / 沙箱保护 / 完全放开，失败时显示稳定诊断码和 apt/dnf 安装提示并保持手动审批；默认均为完全放开 `off`，明确 sandbox 不可用才回退手动审批）。思考内容通过对话内的折叠项直接展开，不提供单独的显示开关。
- **提供商**：
  ...
  - **自定义提供商**：增 / 改 / 删 OpenAI 兼容或 Anthropic 兼容的第三方 provider（id / 名称 / API 类型 / Base URL / API Key），并可逐条配置模型列表——每个模型含 id / 名称 / 是否支持图片 / 支持思考（默认开启）/ 上下文窗口 / 最大输出 token，带字段校验（id 格式与唯一性、token 上限为正且不超过上下文窗口、模型数量上限）。
- **模型**：列出 Agent 当前可用模型，按**提供商分组**（显示提供商名称，无名称才用 id），可逐个开关可见性、搜索。

自定义模型的「支持思考」是二态能力开关，独立于会话的思考强度和思考段的局部展开／折叠状态。关闭时禁用会话思考强度选择并显示说明；重新开启后可选择强度。未填写 `reasoning` 的自定义模型默认开启，显式关闭不会被内置目录覆盖；内置和平台模型仍以目录能力为准。
...
3. GUI 创建 Thread 与 Run，并把用户消息交给 Agent。
...
10. Run 完成后，assistant message、run events、tool calls、tool outputs 由 Agent 持久化（Agent SQLite + session JSONL，唯一真源，见 §4.3–4.7）；GUI 保留自己的 Run 投影与 approval 记录。
