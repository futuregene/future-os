...
对话按 Agent 的父子会话关系展示为最多三层的树（不含 Workspace 标题层），默认收起；点击标题仍直接打开对话。分叉和 loop 派生执行会话都使用同一关系。子对话随根对话分组，即使其工作目录不同；置顶子对话独立进入置顶区并保留自己的子树。缺失、归档或删除的父对话不隐藏存活子对话；更深的历史层级平铺到第三层，不删除或改写 Agent 关系。批量全选包含当前分组中已收起的子对话，删除不自动级联。
