...
手机是 Desktop 的远程查看与控制端，工具在电脑本机执行。远程能力必须可见、可控：GUI 模式关闭 Desktop 即断开远程；显式 `--headless` 模式不创建窗口，在终端前台运行，按需展示平台登录与手机配对的二维码及链接，Ctrl+C 关闭入口。无头模式不是静默隐藏窗口或自动后台保活。两种模式下，Desktop 拥有的 Agent 和进行中的对话都随 Desktop 退出而结束；已有的外部 Agent 不由它终止。
