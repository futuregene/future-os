...
**产品背景与安全边界：远程必须可见、可控。** 手机遥控 AI 可以间接触发本机文件访问、工具调用和命令执行，客户需要一个能直观看到、也能主动关闭的远程入口。Desktop 是这个入口：GUI 模式下用户关闭 Desktop，就应当能够确认本机不再通过手机遥控接受新的远程操作；显式 `--headless` 模式以占用的终端和 Ctrl+C 为可见、可控的运行边界。后台 Agent 的存活不能隐式开启或维持远程能力。这是产品信任边界，不是为了连接稳定性可以放宽的实现细节。
...
- **关闭 Desktop = 远程连接断开。** GUI 用户关闭主窗口或退出应用时，不得静默隐藏窗口并继续接受远程操作；无头模式 Ctrl+C/退出也关闭入口。最小化或切换到其他应用不等于关闭。远程开启、连接中、已断开应在相应界面或终端有明确状态。普通打开 GUI 不等于授权开启远程；显式 `--headless` 表示启动手机远程入口。
...
`futureos --headless` 复用同一 Rust 后端，前台占用终端，不启动 Tauri Builder、窗口、WebView 或 GUI 插件。未登录时通过设备授权流程打印浏览器登录二维码、网址和用户码；不在服务器打开浏览器。授权结果由 Agent 通过原有配置写入路径保存。手机配对是第二阶段：入口与 Agent 就绪后，打印由同一邀请生成的终端二维码及完整 `futureos://remote/pair` 链接，供 App 扫描或粘贴，不是数字短码。
