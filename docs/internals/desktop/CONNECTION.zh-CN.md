...
本文件统一维护 Desktop—平台服务/NATS 网关—移动端的远程连接资料，包括产品背景、现有接线、模块边界、配对与权限、断线恢复、状态同步、技术方案及开发验收。Remote v2 的端到端加密、配对信任、迁移与线协议见 [REMOTE_E2EE.zh-CN.md](REMOTE_E2EE.zh-CN.md)，其安全契约取代本文原先的“信任中继内容”边界。[产品文档](PRODUCT.zh-CN.md)保留功能入口，本文件是该领域设计与契约的唯一维护位置。

审查日期：2026-09-14。源码基线为 FutureOS `2774e4a9` 加本文同一变更候选，以及本地 future-server `bbd6c23`；服务端基线不代表线上实际部署。文中分别标记“现状”“目标”和“待验证”，不得将方案写成已实现的能力。本文不覆盖模型提供商 HTTP、IM 通道等其他网络子系统。
...
**产品背景与安全边界：远程必须可见、可控。** 手机遥控 AI 可以间接触发本机文件访问、工具调用和命令执行，客户需要一个能直观看到、也能主动关闭的远程入口。Desktop 是这个入口：GUI 模式下用户关闭 Desktop，就应当能够确认本机不再通过手机遥控接受新的远程操作；独立 `futureos-headless` 入口以占用的终端和 Ctrl+C 为可见、可控的运行边界。后台 Agent 的存活不能隐式开启或维持远程能力。这是产品信任边界，不是为了连接稳定性可以放宽的实现细节。

- **远程能力归属 Desktop 进程。** 配对、网关连接、远程命令入口及其恢复任务由 Desktop 管理；不迁入常驻 Agent，不通过托盘保活或移动端唤醒绕过 Desktop 生命周期。用户可以显式选择无头前台模式，但不会自动注册服务或转入后台。Agent 仍可服务本地客户端，但其存活不代表手机遥控入口仍然开放。
- **关闭 Desktop = 远程连接断开。** GUI 用户关闭主窗口或退出应用时，不得静默隐藏窗口并继续接受远程操作；无头模式 Ctrl+C/退出也关闭入口。最小化或切换到其他应用不等于关闭。远程开启、连接中、已断开应在相应界面或终端有明确状态。普通打开 GUI 不等于授权开启远程；运行 `futureos-headless` 表示启动手机远程入口。
...
- Remote 状态使用绿点表示已连接、黄点表示正在建立连接或自动恢复、红点表示需要用户处理的断开。黄点只在确有连接或恢复动作运行时出现；失败后没有重试任务时必须为红色，不能用“正在恢复”掩盖静止状态。
- 自动恢复期间不要求用户操作；Desktop 仍运行、远程访问仍被允许、网络恢复且配对仍有效时应自动回到已连接。
- 只有自动恢复停止、服务拒绝连接或配对撤销等终态才展示处理页面。提示固定为“状态、原因、处理方案”三层，例如“配对已失效 / 配对已失效（PA001）/ 请在桌面管理中重新扫码配对”。不向用户暴露 NATS、JWT、连接代、重试次数等内部术语；移动端连接生命周期不再另弹瞬时错误条幅。

手机端（Android / iOS）配对后的体验：

- **配对与首屏**：扫描桌面二维码，或手动粘贴配对码；二维码 5 分钟有效且只能使用一次。启动时先读取本地 Desktop 登记：只要存在登记，无论连接、凭证或撤销状态如何，都稳定进入带 Desktop Header 的已配对页面，连接状态在后台更新；只有完全没有登记才进入扫码页。已配对管理页可以新增扫码，扫码页只提供返回，不再提供反向跳转按钮。
...
`futureos-headless` 复用同一 Rust 后端，前台占用终端，不启动 Tauri Builder、窗口、WebView 或 GUI 插件。未登录时通过设备授权流程打印浏览器登录二维码、网址和用户码；不在服务器打开浏览器。授权结果由 Agent 通过原有配置写入路径保存。手机配对是第二阶段：入口与 Agent 就绪后，打印由同一邀请生成的终端二维码及完整 `futureos://remote/pair` 链接，供 App 扫描或粘贴，不是数字短码。图形入口 `futureos` 不再接受 `--headless`；独立二进制在构建时排除 GUI 依赖。

有效登录和配对均复用，不因每次启动或断网而主动重配。`--no-qr` 仅保留文字链接，窄终端自动回退；`--re-pair` 明确授权替换原配对。首次登录/配对要求交互终端，拒绝把邀请输出到重定向日志。GUI 和无头模式按同一数据目录互斥，防止双重接管。
...
macOS 本地联调脚本同样遵循进程所有权：先停止 Desktop 进程组，再停止由脚本启动的 Agent。正常 Ctrl+C 不把预期的 SIGTERM/npm 退出码打印成错误；构建和 Desktop 的完整输出分别写入 `.logs/future-agent-test.build.log`、`.logs/future-cli-test.build.log` 与 `.logs/futureos-desktop-test.log.console`，只有意外失败才在终端给出日志位置并收紧展示末尾内容。

默认构建同时支持 GUI 与无头入口；`cargo build --release --no-default-features` 提供不链接 Tauri/GTK/WebKit 的服务器构建（仍需匹配的 `future` CLI 提供 Agent）。这不是另一套 Remote SDK 或独立的远程协议实现。手机、平台环境和版本必须匹配。运行说明见 [Desktop 无头模式使用指南](../../guide/desktop-headless.zh-CN.md)（[English](../../guide/desktop-headless.md)）。
...
当前 Desktop 的退出事件调用 Remote 停止，再清理并终止自己拥有的 Agent；运行中的对话另有退出确认与中断流程。这与本文件的 sidecar 产品约定一致。代码依据：[退出入口](../../../desktop/src-tauri/src/lib.rs)、[Agent 所有权与退出](../../../desktop/src-tauri/src/agent_supervisor.rs)。这些入口的存在不等于强杀、崩溃或工具子进程全部终止的实机验证。
...
桌面端显示名由用户在本机设置（可选，随配对索引保存但不上报平台，也不写入
Desktop）；未命名时列表回退显示 Desktop id。凭证刷新会保留该名称，解除配对时随之删除。
...
旧版待确认操作。用户显式解除配对只清理对应 Desktop 的登记与数据；收到撤销通知、凭证失效或凭证损坏时停止连接并清空该 Desktop 的镜像业务状态，但保留本地 Desktop 登记。界面显示红色“配对已失效”，只有用户确认“解除配对”才删除登记，避免把历史操作表现成系统丢失信息。
...
当前配对授权覆盖该 Desktop 的会话，不能误称为逐会话授权。Remote v2 在 NATS 之上使用 Noise 认证密钥协商和逐消息 AEAD，业务消息、文件及状态通知均端到端加密；中继不再承担内容真实性的信任。手机要求 WSS，Desktop 生产连接强制验证 TLS。扫码和粘贴完整邀请保持同一流程，邀请携带仅端侧可用的一次性秘密；原有配对须重新建立，不允许静默回退 v1 明文。中继仍可观察主题、流量元数据并拒绝服务。实现、测试边界和本地密钥存储限制见 [REMOTE_E2EE.zh-CN.md](REMOTE_E2EE.zh-CN.md)。
...
- 原架构重构未将端到端内容加密列为开发依赖；后续 Remote v2 已单独加入端侧加密，无须中继解析密文。即时云端撤销仍不是现有服务端保证。若现有部署不满足安全传输要求，明确报告连接不可用，不静默降级或自行修改服务端。
...
源码入口：[手机连接](../../../mobile/src/remote/client.ts)、[手机生命周期](../../../mobile/src/remote/useRemoteConnection.ts)、[手机配对 HTTP](../../../mobile/src/remote/pairing.ts)、[目录状态](../../../mobile/src/remote/useSessionCatalog.ts)、[Desktop 监督](../../../desktop/src-tauri/src/remote/mod.rs)、[Desktop 命令及握手](../../../desktop/src-tauri/src/remote/commands.rs)。服务端跨仓库入口为 `future-server/platform/service/src/routes/remote.rs`。
...
源码布局与上述边界一致：`remote/mod.rs` 只保留公共门面，诊断、健康、在线状态、发布、传输、测试 Web 服务，以及监督器的状态/启动/停止/状态投影分别位于独立模块；`remote_host/business/` 按目录、历史、提示执行、设置、传输和线协议限额拆分；`agent_bridge/mod.rs` 同样只作为查询、提示、对账和删除发件箱等模块的门面。大型回归测试统一放在各子系统的 `tests.rs`，不再与生产路由交错。
...
- Desktop 从系统休眠恢复时，仅在恢复任务实际运行期间报告黄色 `reconnecting`；恢复任务结束但仍未可用时报告红色失败。Desktop 页面不得出现“等待桌面端”这类角色错误文案，也不在状态卡片下面重复显示连接提示。
...
| 情况 | 策略 |
| --- | --- |
| 手机首次连接或用户手动重新连接 | 单次尝试总预算 20 秒；成功进入已连接，失败进入红色已断开，不套用三分钟自动恢复 |
| 手机已成功连接后的网络、DNS、超时、凭证接口网络失败 | 在允许远程的前提下自动恢复；退避基数为 1、2、4、8、16、30 秒并使用正负 20% 抖动，单次不超过 30 秒，总恢复窗口不超过 3 分钟 |
...
| 配对撤销、凭证失效或凭证损坏 | 终态 `revoked`/配对失效，停止所有连接与重试，清空镜像业务状态但保留本地 Desktop 登记；只有用户显式解除配对才删除登记 |
| 就绪后协议错误或关键订阅/任务失效 | 十分钟内最多三次整代恢复，再次失败进入 `generation_unhealthy`；持续健康 60 秒或用户主动重连重置预算 |
| 就绪前构建失败 | 归属于本次连接尝试；清理候选订阅不得重复消耗关键任务恢复预算 |
| 收到 Desktop 主动断开、休眠、退出或关机通知 | 手机立即停止自动恢复并清空列表/时间线镜像，进入红色独立状态页；唤醒或重新打开后由用户手动重新连接 |
...
stateDiagram-v2
    ...
    连接中 --> 已连接: 握手及就绪成功
    连接中 --> 已断开: 失败或 20 秒超时
    已连接 --> 自动恢复: 瞬时网络异常或心跳过期
    自动恢复 --> 已连接: 恢复成功
    自动恢复 --> 已断开: 总恢复时间超过 3 分钟
    已连接 --> 已断开: Desktop 主动断开、休眠、退出或关机
    已连接 --> 配对已失效: 撤销或凭证失效/损坏
    自动恢复 --> 配对已失效: 恢复时确认配对失效
    已断开 --> 连接中: 手动重新连接
    已连接 --> 扫码: 显式解除配对
    已断开 --> 扫码: 显式解除配对
    配对已失效 --> 扫码: 显式解除配对
...
图中“连接中”和“自动恢复”统一映射为黄色，“已连接”为绿色，“已断开”和“配对已失效”为红色。本地只要保留 Desktop 登记，启动时就先进入已配对页面并在后台更新状态；只有无登记或用户显式解除配对后才进入扫码页。

对外同一时刻最多报告一个就绪连接，候选不作为第二个可操作入口。`failed`、`revoked` 和主动停止状态不能创建连接、刷新凭证或启动连接重试定时器；解绑后的云端撤销补偿独立运行，不能重新启用远程。用户明确重新开启创建新的访问代次；配对失效仍保留本地登记，用户可从 Desktop 管理新增配对，或显式解除旧登记。自动恢复不得清空业务去重记录、已确认配对、同步游标、待查询操作 ID 或故障统计；只有自动恢复超过三分钟进入终态时，才清空当前镜像列表和时间线。每类故障按故障周期汇总，滚动 24 小时内最多输出 16 条，避免断网刷屏与磁盘增长。
...
> 已知的一处冲突：桌面端替换 pre-v2 凭证的日志行复用了 `[PA003]` 标签（`desktop/src-tauri/src/remote/supervisor/start.rs:505`），而本表把 `PA003` 保留给手机「配对领取地址不受信任」提示。上表才是契约；桌面日志标签只是本地调试标记，不是面向用户的代码。
...
| 客户状态 | 颜色 | 含义与恢复方式 |
| --- | --- | --- |
| 已连接 | 绿色 success | 当前连接可用。Desktop 表示远程入口可用，手机还必须确认桌面在线及本机服务可用；不表示所有历史数据已同步 |
| 连接中 | 黄色 warning | 暂不可用且确有连接或自动恢复动作运行；合并首次连接、等待桌面、重连、续期和本机服务恢复。不要求客户理解这些区别 |
| 已断开 | 红色 danger | 已主动停止、明确收到 Desktop 断开通知、配对失效、首次连接失败或自动恢复已终止。普通断开提供重新连接；配对失效只提供显式解除配对 |

展示模型固定分为三层，组件不得自行从底层阶段拼接文案：

1. `level` 是最直观的绿、黄、红可用性，只允许 `connected`、`connecting`、`disconnected`。
...
展示结果同时给出 `action`，限定为等待、检查网络、打开或重启 Desktop、重新连接、解除配对、稍后重试或联系客服。颜色、客户状态、操作和支持码必须由同一个纯转换层一次生成；Desktop 连接页与侧栏、手机 Badge、空状态和输入区只消费该结果。底层仍保留 `connecting`、`reconnecting`、`refreshing` 等阶段，用于执行恢复策略和日志，但不能直接决定客户文案。
...
手机 Header 始终保留 Desktop 选择器、状态圆点和设置入口。圆点点击浮层以及黄色/红色独立状态页都按“状态、描述、处理方案”排列，描述和处理方案必须各自使用独立文本行，不得合并成一段后依赖自动换行。各处使用同一文案映射；描述可附支持码，句末不加句号。配对失效显示“配对已失效”并提供“解除配对”，不得显示“重新连接”。详细错误和阶段保留在控制台日志；局部功能失败不把仍然可用的主连接标红，也不使用连接生命周期的红色条幅。

黄色期间所有远程通信（请求、发送、上传、下载、模型/思考级别及审批变更）冻结，本地浏览、滚动、编辑等操作可以继续。若黄色状态来自正常列表或会话，保持原页面和数据不闪空；若来自首次连接、红色断开页上的手动重连或尚无成功内容，则复用同一个独立状态页，只把固定位置的图标、文案和按钮切换为加载动画。进入红色终态时清空镜像列表/时间线，并在保留 Header 的独立状态页显示原因、方案和操作按钮。
...
### 8.1 客户端实现状态（2026-09-14；2026-09-26 复核代码）

本节所述客户端重构已落库——下表把每个领域映射到对应代码。「验收重点」记录本轮计划在模拟器与真实设备上验证的内容；本文不跟踪该轮验收的结果。future-server 仍不改。
...
| Desktop 生命周期 | 单个 `Supervisor` 拥有用户意图、访问代次、活动运行时、重试预算和后台任务组；停止/休眠统一取消；初始连接与续期共用候选构建和就绪安装；唤醒恢复只在任务运行时报告黄色 | 停止与就绪交错、睡眠唤醒、短断网、网关重启、两端续期 |
| 手机连接 | 单个连接尝试；首次/手动连接限时 20 秒；成功连接后的自动恢复使用正负 20% 抖动退避并限时 3 分钟；显式 Desktop 断开通知停止重试；业务请求统一通过 ready/桌面可用闸门 | 前后台、系统选择器、网络切换、20 秒与 3 分钟边界、迟到恢复不越过主动停止 |
| 状态展示 | Desktop 与手机均由纯转换层一次生成 `level`、`customerState`、`action`、`supportCode`；绿黄红一致；手机 Header、状态浮层、冻结中的列表/会话和独立状态页复用同一结果 | 固定槽位无闪烁；状态、原因、方案及操作一致；原始错误只进控制台 |
| 配对与解绑 | HTTP 有截止时间；凭证双槽提交兼容旧版本；启动路由以 Desktop 登记而非凭证有效性为准；撤销/损坏保留登记并显示配对失效；只有显式解除才删除；云端撤销继续持久补偿 | 离线解绑、进程中断、重配、撤销后首屏不跳扫码、跨环境补偿不误发 |
...
- `RemoteClient` 自己拥有首次连接/恢复截止时间、退避定时器和 15 秒 presence 新鲜度；NATS SDK 自动重连关闭，避免 SDK 与本地状态机各自重试。进入非 ready 或 Desktop 不可用时，统一业务闸门拒绝所有远程通信。
- App 启动先发布 SecureStore 中的 Desktop 登记，再异步读取活动凭证并连接。撤销通知和无效/损坏凭证进入红色配对失效状态，不自动调用 `clearCredentials`；只有用户确认解除或删除某一 Desktop 才移除登记。
