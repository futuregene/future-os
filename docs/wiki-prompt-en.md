...
| `CLI.md` | CLI (`future`) | Optional advanced command-line tool: location, running, command groups |
| `Feishu.md` | Feishu Integration | Connect FutureOS to Feishu (Lark): bot, channel bridge (`future-channel`), slash commands, config |
| `DingTalk.md` | DingTalk Integration | Connect FutureOS to DingTalk: bot, channel bridge (`future-channel`), slash commands, config |
...
>
> **Do not generate `Models.md`**: it is auto-generated by `make generate-models` (`scripts/generate_models.py` writes `docs/wiki/{en,zh}/Models.md`) — never hand-write it, and don't include it in the sidebar.
...
- Common built-in skills table (skill name + purpose, **reference only — use the actual All tab list from the app**): Account (profile/quota/recharge), Web (search the web and read full articles), Paper (search PubMed/ArXiv/DOI and fetch full text), Deep research (multi-source cross-checking, cited reports), Document (PDF/Word to structured text), Image (generate/edit/analyze images, including OCR), Loop (long-running goal orchestration: the `/future-loop` skill drives the `future loop` control plane), Browser (drive a browser: open pages, click, type, screenshot), Database lookup (query public databases), Experimental design (experiment design), Peer review (structured review), Scientific writing (scientific writing), Slides (generate slides), Software install (software installation), Skill creator (help make new skills).
...
- Positioning: optional CLI tool `future`, shipped with every download; the desktop app already covers most needs — use the CLI when you want scripting / automation / pure-terminal workflows. Start with a note that terminal-shy users can skip this page.
...
- **Command groups** (use `cli/src/index.ts` actual dispatch; there is no `tui` group and no `channel` group):
  ...
  - `account`: profile and balance (`profile` / `balance`)
  - `run`: send a one-shot prompt and print the answer (give examples: direct question, `--model`, `@file`, pipe input; explain `@<path>` includes files, common options `--model` (supports `model:thinking`, e.g. `sonnet:high`), `--thinking <level>`, `--continue`/`-c`, `--fork <entry-id>`, `--session <id>`, `--no-session`, `--permission <level>`, `--mode json`, `--cwd <dir>`)
...
4. **CLI name**: use `future` throughout, never `future-cli` (check commands, paths, and examples).
