...
**B —— 使用已知 Provider。** 将 API Key 放入 `~/.future/agent/auth.json`，按 Provider 名索引。多数 Provider 已有内置 Base URL，模型自动发现：
...
**C —— 自定义 Provider。** 需要手动指定模型列表时，使用 `~/.future/agent/models.json`：
