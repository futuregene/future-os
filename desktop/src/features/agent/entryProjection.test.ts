...
describe("entriesToMessages", () => {
    ...
    const nodes = entriesToTurns([
      {
        ...
        blocks: [{ kind: "text", text: "hello" }],
        runId: "run-user",
        run: { status: "failed", error: "must not attach" },
      },
      {
        id: "a1",
        kind: "assistant",
    ...
    const entries: SessionEntry[] = [
      {
        ...
        kind: "user",
        role: "user",
        createdAtMs: Date.parse("2026-07-01T10:00:00+08:00"),
        blocks: [{ kind: "text", text: "hi" }],
      },
      {
        id: "a1",
        kind: "assistant",
        role: "assistant",
        createdAtMs: Date.parse("2026-07-01T10:00:07+08:00"),
        blocks: [{ kind: "text", text: "hello" }],
  ...
  it("leaves usage undefined when the agent reported none (no footer shown)", () => {
    const entries: SessionEntry[] = [
      ...
      },
      {
        ...
        kind: "assistant",
        role: "assistant",
        createdAtMs: Date.parse("2026-07-01T10:00:01+08:00"),
        blocks: [{ kind: "text", text: "hello" }],
      },
    ];
  ...
  it("sets a write tool activity's target to the file path from its args", () => {
    const entries: SessionEntry[] = [
      {
        ...
        role: "user",
        createdAtMs: Date.parse("2026-07-01T10:00:00+08:00"),
        blocks: [{ kind: "text", text: "write a file" }],
      },
      {
        id: "a1",
        kind: "assistant",
        role: "assistant",
        createdAtMs: Date.parse("2026-07-01T10:00:05+08:00"),
        blocks: [
          { kind: "text", text: "done" },
          ...[
            {
              ...
              function: {
                name: "write",
                arguments: JSON.stringify({ path: "poem.md", content: "..." }),
              },
            },
          ].map(call => ({
            kind: "tool_call",
    ...
    const activity = assistant?.segments?.find(
      segment => segment.kind === "activity",
    );
    expect(
      activity?.kind === "activity" ? activity.item.target : undefined,
    ).toBe("poem.md");
  });
  ...
  it("does not duplicate a tool activity for the tool result entry", () => {
    const entries: SessionEntry[] = [
      ...
      },
      {
        ...
        kind: "assistant",
        role: "assistant",
        createdAtMs: Date.parse("2026-07-01T10:00:03+08:00"),
        blocks: [
          { kind: "text", text: "" },
          ...[
            {
              ...
              function: {
                name: "write",
                arguments: JSON.stringify({ path: "poem.md" }),
              },
            },
          ].map(call => ({
            kind: "tool_call",
      ...
      // The agent's tool result entry for the same call — must not add a second row.
      {
        ...
        kind: "tool",
        role: "tool",
        createdAtMs: Date.parse("2026-07-01T10:00:04+08:00"),
        blocks: [
          {
            kind: "tool_result",
            toolCallId: "call_00_test",
            text: "Written to poem.md",
            isError: false,
          },
        ],
      },
    ...
    const assistant = entriesToMessages(entries)[1];
    const activities
      = assistant?.segments?.filter(segment => segment.kind === "activity")
        ?? [];
  ...
  it("marks a tool activity failed when its result reports an error", () => {
    const entries: SessionEntry[] = [
          ...
          ...[
            {
              ...
              function: {
                name: "shell",
                arguments: JSON.stringify({ command: "futre --version" }),
              },
            },
          ].map(call => ({
            kind: "tool_call",
      ...
      },
      {
        ...
        role: "assistant",
        createdAtMs: Date.parse("2026-07-01T10:00:05+08:00"),
        blocks: [{ kind: "text", text: "that failed" }],
      },
    ];
    ...
    expect(
      activity?.kind === "activity" ? activity.item.status : undefined,
    ).toBe("failed");
  });
          ...
          ...[
            {
              ...
              function: {
                name: "shell",
                arguments: JSON.stringify({ command: "grep foo file.txt" }),
              },
            },
          ].map(call => ({
            kind: "tool_call",
  ...
  it("orders preamble text before the tool activity it introduces", () => {
    const entries: SessionEntry[] = [
      {
        ...
        role: "user",
        createdAtMs: Date.parse("2026-07-01T10:00:00+08:00"),
        blocks: [{ kind: "text", text: "check config" }],
      },
      {
        id: "a1",
        kind: "assistant",
          ...
          ...[
            {
              ...
              function: {
                name: "read",
                arguments: JSON.stringify({ path: "config.toml" }),
              },
            },
          ].map(call => ({
            kind: "tool_call",
    ...
    const kinds = entriesToMessages(entries)[1]?.segments?.map(
  ...
  it("collapses a burst of same-kind tools into one row with a count", () => {
    const editCall = (path: string) => ({
      id: "call_00_test",
      function: { name: "edit", arguments: JSON.stringify({ path }) },
    });
    const entries: SessionEntry[] = [
      {
        id: "u1",
        ...
        role: "user",
        createdAtMs: Date.parse("2026-07-01T10:00:00+08:00"),
        blocks: [{ kind: "text", text: "edit files" }],
      },
      {
        id: "a1",
        kind: "assistant",
        ...
        blocks: [
          { kind: "text", text: "" },
          ...[editCall("a.ts"), editCall("b.ts"), editCall("c.ts")].map(
            call => ({
              kind: "tool_call",
              toolCallId: call.id,
              name: call.function.name,
    ...
    const activities
      = entriesToMessages(entries)[1]?.segments?.filter(
        s => s.kind === "activity",
      ) ?? [];
    expect(activities).toHaveLength(1);
    const item
      = activities[0]?.kind === "activity" ? activities[0].item : undefined;
  ...
  it("renders the compaction summary as a divider, not a user bubble", () => {
    const entries: SessionEntry[] = [
      {
        ...
        blocks: [
          {
            kind: "text",
            text: "[Context compaction: Previous conversation summarized. Files read: a.ts. Modified: .]",
          },
        ],
      },
      {
        ...
        kind: "user",
        role: "user",
        createdAtMs: Date.parse("2026-07-01T10:01:00+08:00"),
        blocks: [{ kind: "text", text: "carry on" }],
      },
      {
        id: "a1",
        kind: "assistant",
        role: "assistant",
        createdAtMs: Date.parse("2026-07-01T10:01:02+08:00"),
    ...
    const divider = messages.find(message =>
      message.segments?.some(s => s.kind === "compaction"),
    );
  ...
  it("derives stable ids from entry ids across re-projections", () => {
    ...
    const entries: SessionEntry[] = [
      {
        ...
        kind: "user",
        role: "user",
        createdAtMs: Date.parse("2026-07-01T09:00:00+08:00"),
        blocks: [{ kind: "text", text: "[Context compaction: summarized]" }],
      },
      {
        id: "u1",
        kind: "user",
    ...
    expect(second.map(m => m.segments?.map(s => s.id))).toEqual(
      first.map(m => m.segments?.map(s => s.id)),
    );
    ...
    const messages = entriesToMessages([
      {
        ...
        role: "user",
        createdAtMs: 0,
        blocks: [{ kind: "text", text: "see this" }],
        metadata: {
          attachments: [
            {
              path: "/a.png",
  ...
  it("treats an empty tool result as completed and an Error: result as failed", () => {
    const entries: SessionEntry[] = [
      ...
      },
      {
        ...
        createdAtMs: 0,
        blocks: [
          { kind: "tool_result", toolCallId: "c1", text: "", isError: false },
        ],
      },
      {
        id: "t2",
    ...
    const activities = segments.flatMap(s =>
      s.kind === "activity" ? [s.item] : [],
    );
  ...
  it("synthesizes an id for tool calls without one", () => {
    const messages = entriesToMessages([
      ...
      },
      {
        ...
        blocks: [
          { kind: "text", text: "" },
          ...[{ id: "", function: { name: "read", arguments: "{}" } }].map(
            call => ({
              kind: "tool_call",
              toolCallId: call.id,
              name: call.function.name,
    ...
    expect(activity && "item" in activity && activity.item.id).toBe("seg_a1_1");
  });
    ...
    const messages = entriesToMessages([
      {
        ...
        role: "assistant",
        createdAtMs: 0,
        blocks: [{ kind: "text", text: "orphan reply" }],
      },
    ]);
  ...
  it("flushes an open exchange when a compaction divider arrives", () => {
    const messages = entriesToMessages([
      ...
      },
      {
        ...
        role: "user",
        createdAtMs: 0,
        blocks: [{ kind: "text", text: "[Context compaction: summary]" }],
      },
      {
        id: "u2",
        kind: "user",
    ...
    expect(
      messages.some(m => m.segments?.some(s => s.kind === "compaction")),
    ).toBe(true);
