...
20 MiB、最多 10 个附件/4 张图片），图片最长边超过 2000px 直接拒绝。JPEG/BMP 在
手机端以 JPEG 质量 65 编码后上传，PNG/WebP/GIF 保持原文件；界面只展示原始大小。
手机仅在点击时下载桌面附件，移动网络和未知网络类型需二次确认。图片预览最长边
600px（GIF 不提供移动预览），Markdown 最多完整渲染 2 MiB；预览按内容哈希缓存并
...
- Remote（当前前端 `ActivityRail.tsx` 在 release 构建隐藏入口；后端 `remote_start` 门禁本阶段暂缓，进入正式发布前需随 L1 scoped JWT 策略一并收口）。
- 置顶分区（所有置顶对话）。
- Workspace 列表。
- Workspace 下子对话。
- Chat 列表。
