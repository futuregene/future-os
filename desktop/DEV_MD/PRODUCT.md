...
对话按 Agent 的父子会话关系展示为最多三层的树（不含 Workspace 标题层），展开箭头样式与工作区标题的折叠箭头一致（位置规则见下段），默认收起；点击标题仍直接打开对话。分叉和 loop 派生执行会话都使用同一关系。子对话随根对话分组，即使其工作目录不同；置顶子对话独立进入置顶区并保留自己的子树。缺失、归档或删除的父对话不隐藏存活子对话；更深的历史层级平铺到第三层，不删除或改写 Agent 关系。批量全选包含当前分组中已收起的子对话，删除不自动级联。

标题空间优先：无子对话的行不预留展开箭头占位，子级行保留占位以保持同级标题对齐。同时普通对话和置顶区（非工作区）的标题与高亮左缘之间保留 16px 起始留白，标题不贴边。展开箭头均保留在左侧、且位于本行内容起点：工作区内的会话箭头随工作区缩进内移，与工作区分组标题的折叠箭头分属不同列，不会被误认成「收起工作区」。其他会话新增或展开子对话不改变无关顶层行的标题起点。
