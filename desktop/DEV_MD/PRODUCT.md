...
子对话的展开开关必须是 **+ / − 树节点开关**（收起 `+`、展开 `−`），与工作区 / 分区标题的折叠 **chevron 在图标和位置上都不同**：两者位置不同时不得同列，同列时（移动端受 44px 触控区限制）必须仅靠图标就能分清；不得用同一图标、也不得用与全站细描边语言不一致的实心图形。

标题空间优先：无子对话的行不预留展开箭头占位，子级行保留占位以保持同级标题对齐。同时普通对话和置顶区（非工作区）的标题与高亮左缘之间保留 16px 起始留白，标题不贴边。展开开关保留在左侧、随所在分组缩进；其他会话新增或展开子对话不改变无关顶层行的标题起点。
