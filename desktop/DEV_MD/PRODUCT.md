...
标题空间优先：无子对话的行不预留展开箭头占位，子级行保留占位以保持同级标题对齐。同时普通对话和置顶区（非工作区）的标题与高亮左缘之间保留 16px 起始留白，标题不贴边。子对话的展开控件与工作区 / 分区标题的折叠控件**在位置和图标上都不同**：它是实心三角（未展开 ▸、展开旋转为 ▾），固定在本行内容起点，随所在分组缩进；标题 / 分区用 chevron。两者不得同列、同图标，否则会被误读成同一个折叠控件。其他会话新增或展开子对话不改变无关顶层行的标题起点。
