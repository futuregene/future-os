...
本功能的完整产品背景、交互规则、网络接线、配对与权限、架构方案、自恢复与同步契约、支持码及开发计划统一维护在 [远程连接设计](CONNECTION.md)，本节不重复维护。
...
回复结束必须向用户交代，沿用消息内状态分割线，不用整页弹窗或空白回复掩盖错误：主动停止、输出长度限制及压缩流程维持各自提示；异常按有证据的模型、网络/上游连接、超时或软件/后台服务原因展示，并保留已生成内容。缺少正常结束信号但没有足够归因证据时显示“原因未确认”，不能一律归咎模型或网络。各协议认可的正常结束、拒答、内容过滤、暂停分别处理，不要求所有模型发送同一种结束标记。具体判定见 [回复结果语义](../../docs/response-outcomes.zh-CN.md)。
