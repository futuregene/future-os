...
输入框的 `/` 菜单不只承载 Skill，也承载少量高频的**上下文工具**。上下文工具执行 FutureOS 自身的会话操作，不作为用户 prompt 发送给模型，也不伪装成 Skill。首个上下文工具是「压缩 / Compact」：仅在已有 Agent session 的对话中提供，选择后立即触发 standalone 上下文压缩。它不创建用户消息、普通 Agent 回复或新的 Run；摘要请求是该操作唯一的模型通信。手动压缩跳过自动阈值，成功 checkpoint 会记录 `trigger: manual`，并通过“你手动压缩了此对话的上下文”分割线展示用户自己的选择；失败也会在原位反馈。
