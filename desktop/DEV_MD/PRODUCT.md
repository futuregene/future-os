...
输入框的 `/` 菜单不只承载 Skill，也承载少量高频的**上下文工具**。上下文工具执行 FutureOS 自身的会话操作，不作为用户 prompt 发送给模型，也不伪装成 Skill。首个上下文工具是「压缩 / Compact」：仅在已有 Agent session 的对话中提供，选择后立即触发 standalone 上下文压缩，并通过对话内分割线显示正在压缩、成功或失败。
