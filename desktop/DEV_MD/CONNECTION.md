...
审查日期：2026-09-14。源码基线为 FutureOS `c18d1063` 加本文同一变更候选，以及本地 future-server `bbd6c23`；服务端基线不代表线上实际部署。文中分别标记“现状”“目标”和“待验证”，不得将方案写成已实现的能力。本文不覆盖模型提供商 HTTP、IM 通道等其他网络子系统。
...
手机 Header 始终保留 Desktop 选择器、状态圆点和设置入口。圆点点击浮层以及红色独立状态页都按“状态、原因、处理方案”排列，使用同一文案映射；原因可附支持码，句末不加句号。配对失效显示“配对已失效”并提供“解除配对”，不得显示“重新连接”。详细错误和阶段保留在控制台日志；局部功能失败不把仍然可用的主连接标红，也不使用连接生命周期的红色条幅。
