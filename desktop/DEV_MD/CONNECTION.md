...
本文件统一维护 Desktop—平台服务/NATS 网关—移动端的远程连接资料，包括产品背景、现有接线、模块边界、配对与权限、断线恢复、状态同步、技术方案及开发验收。Remote v2 的端到端加密、配对信任、迁移与线协议见 [REMOTE_E2EE.md](REMOTE_E2EE.md)，其安全契约取代本文原先的“信任中继内容”边界。[产品文档](PRODUCT.md)保留功能入口，本文件是该领域设计与契约的唯一维护位置。
...
当前配对授权覆盖该 Desktop 的会话，不能误称为逐会话授权。Remote v2 在 NATS 之上使用 Noise 认证密钥协商和逐消息 AEAD，业务消息、文件及状态通知均端到端加密；中继不再承担内容真实性的信任。手机要求 WSS，Desktop 生产连接强制验证 TLS。扫码和粘贴完整邀请保持同一流程，邀请携带仅端侧可用的一次性秘密；原有配对须重新建立，不允许静默回退 v1 明文。中继仍可观察主题、流量元数据并拒绝服务。实现、测试边界和本地密钥存储限制见 [REMOTE_E2EE.md](REMOTE_E2EE.md)。
