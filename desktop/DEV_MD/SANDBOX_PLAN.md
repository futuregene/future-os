...
Codex 的 legacy unelevated 后端还把 logon SID 与 Everyone 放入 restricting SID 集以兼容部分 Windows 对象；其包含 token User SID 的 helper 明确用于 elevated 后端的独立沙盒账号。Codex 另外把 logon、Everyone 与 capability 写入 token default DACL，并在交互 window station 中创建只授予 logon SID 的私有 desktop。FutureOS 真机实验确认 capability + logon + Everyone 仍不足以初始化 Windows PowerShell，而加入真实登录用户 SID 后可以启动；但真实 User SID 会命中普通用户文件 ACL，可能直接绕过 workspace 外写边界，因此不得作为兼容性补丁加入 `SidsToRestrict`。FutureOS 保持最小 capability restricting 集，并通过两处局部兼容处理满足双重检查：restricted token 的 default DACL 包含“当前用户 + capability”，只作用于子进程新建的 mutex/event/pipe 等对象；每次启动创建独立 window station + desktop，并只在这两个短生命周期 USER 对象上赋予同样的双 SID ACL。真实文件写边界仍只由 capability ACE 决定。这两点是基于当前 unelevated 单账号威胁模型对 Codex 做法的有意差异，不宣称已经解决所有主机上的 PowerShell 初始化依赖；最终支持状态仍由真机矩阵决定。
...
- `sandbox/windows/token.rs`：创建并验证 `WRITE_RESTRICTED` primary token；restricting 集只含 capability，不加入真实 User SID。恢复 `SeChangeNotifyPrivilege` 以允许路径遍历，并把 token default DACL 收敛为当前用户 SID + capability SID，使子进程自建内核对象同时通过普通与 restricting 两次检查，但不修改任何已有文件 ACL。
