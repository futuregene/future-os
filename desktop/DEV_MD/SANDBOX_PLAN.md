...
- `sandbox/windows/token.rs`：创建并验证 `WRITE_RESTRICTED` primary token；restricting 集含 capability + logon SID + Everyone，但不加入真实 User SID（避免命中普通文件 ACL 破坏写边界）。恢复 `SeChangeNotifyPrivilege` 以允许路径遍历，并把 token default DACL 收敛为当前用户 SID + logon SID + Everyone + capability SID，使子进程自建内核对象同时通过普通与 restricting 两次检查，但不修改任何已有文件 ACL。
