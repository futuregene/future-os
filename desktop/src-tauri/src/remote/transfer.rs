...
pub fn spawn_transfer_loop(
    ...
    spawn_transfer_loop_with_ready(
        client,
        pair_id,
        active,
        super::secure::Transport::legacy_fixture(),
        None,
    )
}

pub(super) fn spawn_transfer_loop_with_ready(
    client: async_nats::Client,
    pair_id: String,
    active: std::sync::Arc<std::sync::atomic::AtomicBool>,
    security: super::secure::Transport,
        ...
        loop {
            tokio::select! {
                ...
                next = sub.next() => {
                    ...
                    let Ok((plaintext, reply_security)) = security.open(&message.subject, &message.payload) else { continue; };
                    if !security.is_active(&reply_security) { continue; }
                    let suffix = message
                        .subject
                    ...
                    let response = match parts.as_slice() {
                        [transfer_id, "chunk", index] => index
                            ...
                            .and_then(|index| super::host().upload_chunk(transfer_id, index, &plaintext)),
                    ...
                    if let Some(reply) = message.reply {
                        ...
                        if let Ok(wire) = reply_security.seal(&bytes) {
                            let _ = client.publish(reply, wire.into()).await;
                        }
                    }
                }
            }
...
async fn publish_download_chunk(
    ...
    super::secure::publish(client, security, subject, bytes).await?;
    client
        .flush()
        .await
