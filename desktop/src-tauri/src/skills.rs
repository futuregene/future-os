...
fn extract_skill_zip(bytes: &[u8], dest: &Path) -> Result<(), AppError> {
    ...
    // Keep incomplete candidates outside the agent's skills discovery tree.
    let parent = dest
        .parent()
    ...
    flatten_single_subdir(&candidate)?;
    let entry = std::fs::read_to_string(candidate.join("SKILL.md"))
        .map_err(|error| AppError::Message(format!("Invalid skill package: {error}")))?;
    if entry.trim().is_empty() {
        return Err("Invalid skill package: empty SKILL.md".into());
    }
    replace_skill_dir(&candidate, dest)?;
    Ok(())
}

/// Same on-disk transaction/lock protocol as the CLI, so the two installers
/// cannot interleave directory replacements. A crash leaves a recoverable backup.
...
mod tests {
    ...
    #[test]
    fn invalid_updates_preserve_previous_skill() {
        let temp = tempfile::tempdir().unwrap();
        let dest = temp.path().join("skills/example");
        std::fs::create_dir_all(&dest).unwrap();
        std::fs::write(dest.join("SKILL.md"), "previous").unwrap();
        for bytes in [
            b"not zip".to_vec(),
            make_zip(&[("other.txt", b"x")]),
            make_zip(&[("SKILL.md", b" ")]),
        ] {
            assert!(extract_skill_zip(&bytes, &dest).is_err());
            assert_eq!(
    ...
    fn skill_commit_rolls_back_recovers_and_excludes_competing_installers() {
        ...
        let backup = temp.path().join(".skill-example.previous");
        ...
        let lock = std::fs::OpenOptions::new()
            .write(true)
            .open(temp.path().join("skills/.example.install.lock"))
            .unwrap();
        lock.lock().unwrap();
