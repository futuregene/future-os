...
import { DEFAULT_CHANNEL_LAUNCHD_LABEL, DEFAULT_CHANNEL_SYSTEMD_UNIT, DEFAULT_CHANNEL_WINDOWS_SERVICE, } from "../constants.js";
...
export async function channel(command) {
...
export function isChannelCommand(command) {
    return command === "start" || command === "stop" || command === "restart" || command === "status";
}
async function runDarwinChannelCommand(command) {
...
async function ensureDarwinLaunchAgent(label, target) {
    ...
    if (result.code !== 0) {
        ...
        throw new Error(`Failed to bootstrap Future channel launchd service (${label}).${output ? `\n${output}` : ""}`);
    }
}
...
async function runLinuxChannelCommand(command) {
...
async function runWindowsChannelCommand(command) {
...
async function resolveChannelBinary() {
    ...
    throw new Error("Future channel binary not found. Build it first with `cd future-os && make build-channel`, or set FUTURE_CHANNEL_BIN.");
}
...
function channelCommandPastTense(command) {
    if (command === "start") {
        return "Started";
    }
    if (command === "stop") {
