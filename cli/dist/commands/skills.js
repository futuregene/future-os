...
const SKILL_BUNDLES = {
    core: {
        description: "Full rare disease & genomics suite. All 14 tools for disease search, phenotype analysis, gene/variant lookup, and literature search.",
        tools: [
...
function skillsGet(name) {
    const bundle = SKILL_BUNDLES[name];
    if (!bundle) {
        console.error(`Unknown skill "${name}". Use "future-cli skills list" to see available bundles.`);
        process.exitCode = 1;
        return;
    }
