...
export function mcpPost(url, method, params, apiKey, sessionId, id) {
    const body = {
        jsonrpc: "2.0",
        method,
        params,
            ...
            res.on("end", () => {
                for (const line of data.split("\n")) {
                    if (line.startsWith("data:")) {
                        ...
                        if (p) {
                            try {
                                resolve({ body: JSON.parse(p), sessionId: sid ?? null });
                                return;
                            }
                            catch {
...
export function mcpNotify(url, method, params, apiKey, sessionId) {
...
export async function initializeSession(apiKey) {
    const url = mcpUrl();
