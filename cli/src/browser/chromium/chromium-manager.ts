 ...
 *
 * Reuses the same launch logic as the current browserStart().
 */
...
export class ChromiumManager implements BrowserManager {
  ...
  async start(options: BrowserLaunchOptions = {}): Promise<InternalBrowserStartResult> {
    ...
    if (!discovered) {
      throw new BrowserNotFoundError(
        "Could not find Chrome or Edge. Pass executablePath to browser start.",
      );
    }
  ...
  async status(connection: BrowserConnectionConfig): Promise<BrowserStatusResult> {
    ...
    try {
      const response = await fetch(`${connection.endpoint}/json/version`, {
  ...
  private _toCdpKind(): "chrome" | "edge" | "chromium" {
    if (this.kind === "safari") return "chrome";
    return this.kind;
  }
}
...
async function resolvePort(requestedPort: number): Promise<number> {
  if (await endpointReachable(`http://127.0.0.1:${requestedPort}`)) {
