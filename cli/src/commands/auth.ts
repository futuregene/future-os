...
export async function login(platformUrlOverride?: string): Promise<void> {
  ...
  const device = await post<DeviceCodeResponse>(platformUrl, "/platform/v1/oauth/device/code", {
  ...
  while (Date.now() - startedAt < device.expires_in * 1000) {
    ...
    const response = await fetch(`${platformUrl}/platform/v1/oauth/device/token`, {
      method: "POST",
