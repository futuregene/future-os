...
export async function agent(command: AgentCommand): Promise<void> {
...
export function isAgentCommand(command: string | undefined): command is AgentCommand {
  return command === "start" || command === "stop" || command === "restart" || command === "status";
}

async function runDarwinAgentCommand(command: AgentCommand): Promise<void> {
...
async function ensureDarwinLaunchAgent(label: string, target: string): Promise<void> {
  ...
  if (result.code !== 0) {
    ...
    throw new Error(
      `Failed to bootstrap Future agent launchd service (${label}).${output ? `\n${output}` : ""}`,
    );
  }
}
...
async function runLinuxAgentCommand(command: AgentCommand): Promise<void> {
...
async function runWindowsAgentCommand(command: AgentCommand): Promise<void> {
...
async function resolveAgentBinary(): Promise<string> {
  ...
  const onPath = await which("future-agent");
  ...
  throw new Error(
    "Future agent binary not found. Build it first with `cd future-os && make build-agent`, or set FUTURE_AGENT_BIN.",
  );
}
...
function agentCommandPastTense(command: AgentCommand): string {
  if (command === "start") {
    return "Started";
  }
  if (command === "stop") {
