...
Rust (agent, channel) + TypeScript (TUI). The Rust agent is the backend; the TS TUI provides the terminal interface. The channel binary bridges external messaging platforms (Feishu/Lark) to the agent via gRPC.
...
make build              # Build agent, TUI, and channel
...
The Rust binary (`future-agent`) is the backend, always running as a gRPC server at `127.0.0.1:50051`. The TS TUI and channel bridge both connect to it via gRPC. Agent CLI has only one flag: `--grpc-addr`.
...
```
                  ...
                  ┌──────────┴───────────────┐
                  │           │              │
           TypeScript TUI    Channel      External clients
           (tui/ via gRPC)  (channel/)    (via gRPC API)
```
