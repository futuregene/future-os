...
| `tools` | `list`, `call` | MCP client — lists and calls tools from a remote MCP server (defaults to `FUTURE_API_BASE`/mcp, or `http://api.westlakefuturegene.com/mcp`). Override with `FUTURE_MCP_URL`. |
