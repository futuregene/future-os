...
Rust (agent, channel) + TypeScript (TUI, CLI). The Rust agent is the backend; the TS TUI provides the terminal interface. The TS CLI (`future-cli`) handles auth, service management, MCP tool calls, and skill bundle discovery. The channel binary bridges external messaging platforms (Feishu/Lark) to the agent via gRPC.
...
make build              # Build agent, TUI, and CLI
...
make build-channel      # Build channel bridge
make build-channel-release  # Build channel bridge (optimized)
make test               # Run all Rust tests
make lint               # Lint Rust + TypeScript (both TUI and CLI)
...
make run-channel        # Build and run channel bridge
...
cd channel && cargo run                # Start channel bridge (connects to agent gRPC)
cd channel && cargo build              # Build channel bridge (debug)
cd channel && cargo build --release    # Build channel bridge (optimized)
...
# Channel proto: channel/proto/feishu_ws.proto (pbbp2 frames) + proto/future.proto (gRPC client)
```

The Rust binary (`future-agent`) is the backend, always running as a gRPC server at `127.0.0.1:50051`. The TS TUI and channel bridge both connect to it via gRPC. The Rust binary itself has only one CLI flag: `--grpc-addr`.
...
Channel config is under `~/.future/channel/`:
...
```
           ...
           (tui/ via gRPC)   (channel/)          (cli/ future-cli)
...
| `tools` | `list`, `call` | MCP client — lists and calls tools from a remote MCP server (default `http://localhost:7003/mcp`). Tool catalog includes 14 rare-disease/genomics tools (case_searcher, disease_searcher, phenotype_analyzer, etc.) |
