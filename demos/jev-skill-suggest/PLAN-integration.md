...
> 状态：**待你确认**。这份是设计 + 拆解，先不动产品代码（`demos/` 之外）。确认后按"交付顺序"开工。
> 建议：实现落在**新的 worktree** `feat/skill-reco`，别和 demo 分支混在一起。若 main 已含
> demo（`27cac777`），逻辑/常量可以 `bench/second-call.mjs` + `suggest.mjs` 的写法为参照。
