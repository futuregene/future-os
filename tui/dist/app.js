...
import { AutocompletePopup, AutocompleteManager, SlashCommandProvider, FilePathProvider, AttachmentProvider } from "./components/autocomplete.js";
import { Editor } from "./components/editor.js";
import { NodeTerminal, SYNC_BEGIN, SYNC_END, Container, resolveOverlayLayout, isFocusable, } from "./tui.js";
import { DARK_THEME, fg, bold } from "./theme.js";
...
export class App extends Container {
    ...
    acManager = new AutocompleteManager();
    ...
    static SEGMENT_RESET = "\x1b[0m\x1b]8;;\x07"; // SGR reset + OSC 8 close (prevents hyperlink leak)
    previousWidth = 0;
    previousHeight = 0;
    ...
    setClearOnShrink(enabled) { this.clearOnShrink = enabled; }
    ...
    getFullRedrawCount() { return this.fullRedrawCount; }
    constructor(grpcAddr = "localhost:50051", cliOptions = {}) {
        super();
    ...
    handleAgentEvent(event) {
        switch (event.type) {
            case "text_chunk":
                ...
                this.chat.appendToLastMessage((event.text ?? ""));
                break;
            case "agent_end": {
            ...
            case "usage": {
                const e = event;
                if (e.input_tokens !== undefined)
                    this.state.tokensIn += e.input_tokens;
                if (e.output_tokens !== undefined)
                    this.state.tokensOut += e.output_tokens;
                break;
            }
            default:
                break;
    ...
    // ─── Welcome Screen ─────────────────────────────────────────────────
    showWelcome() {
        const dim_ = (t) => fg(245, t);
        const sectionHdr = (t) => fg(221, t);
        const add = (content) => {
            this.chat.addMessage({ id: crypto.randomUUID(), role: "system", welcome: true, content });
        };
        const addColored = (content, color) => {
    ...
    async refresh() {
        try {
            ...
            if (s.sessionId && s.sessionId !== this.state.sessionId) {
                this.client.currentSessionId = s.sessionId;
            }
        }
        catch {
    ...
    showHelpOverlay() {
        const helpComponent = {
            render: (width) => this.renderHelp(width),
            invalidate: () => { },
        };
    ...
    getTopOverlay() {
        for (let i = this.overlayStack.length - 1; i >= 0; i--) {
            if (!this.overlayStack[i].hidden)
                return this.overlayStack[i].component;
        }
        return null;
    }
    compositeOverlays(base, termW, termH) {
        ...
        for (const entry of visible) {
            ...
            for (let i = 0; i < maxRows; i++) {
                ...
                if (targetRow >= 0 && targetRow < lines.length) {
                    lines[targetRow] = this.compositeLineAt(lines[targetRow], overlayLines[i], layout.col, layout.width);
                }
            }
        }
        return lines;
    }
    compositeLineAt(base, overlay, col, width) {
        if (isImageLine(base))
            return base;
    ...
    isOverlayVisible(entry, termW, termH) {
        if (entry.hidden)
            return false;
        if (entry.options?.visible)
            return entry.options.visible(termW, termH);
        return true;
    }
    setFocus(component) {
    ...
    requestRender(force = false) {
        if (force) {
            ...
            process.nextTick(() => {
                if (!this.running || !this.renderRequested)
                    return;
                this.renderRequested = false;
    ...
    scheduleRender() {
        if (!this.running || this.renderTimer || !this.renderRequested)
            return;
    ...
    renderHelp(W) {
        const lines = [];
        const dim_ = (t) => fg(245, t);
        const acc = (t) => fg(151, t);
        const bold_ = (t) => fg(252, bold(t));
