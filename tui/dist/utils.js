...
function isRegionalIndicatorPair(s) {
    return REGIONAL_INDICATOR.test(s);
}
function isKeycapSequence(s) {
    if (s.length < 2)
        return false;
...
function isEmojiFlag(s) {
    // Regional indicator pairs
    if (isRegionalIndicatorPair(s))
        return true;
    // Flag emoji tag sequences: U+1F3F4 + tags + U+E007F
    if (s.codePointAt(0) === 0x1F3F4)
        return true;
    return false;
}
function isEmojiModifierBase(code) {
    return ((code >= 0x261D && code <= 0x270F) || // misc symbols
        code === 0x1F385 || // Santa
...
function isExtendedPictographic(code) {
    return ((code >= 0x1F300 && code <= 0x1F64F) || // misc symbols, emoticons
...
function hasEmojiPresentation(s) {
...
const visibleWidthCache = new Map();
...
export function clearVisibleWidthCache() {
    visibleWidthCache.clear();
    graphemeWidthCache.clear();
}
// ─── Tab Handling ──────────────────────────────────────────────────────────
export function replaceTabs(s, tabWidth = 3) {
...
export function normalizeTerminalOutput(s) {
    s = replaceTabs(s);
    // Decompose Thai/Lao AM vowels into base + combining marks for
    // terminal compatibility (matching pi's NFD-based approach).
    if (!THAI_LAO_AM_REGEX.test(s))
        return s;
    return s.replace(THAI_LAO_AM_GLOBAL_REGEX, (char) => char === "\u0e33" ? "\u0e4d\u0e32" : "\u0ecd\u0eb2");
}
export function extractAnsiCode(s, pos) {
    if (pos >= s.length)
        return null;
    if (s[pos] !== "\x1b")
        return null;
...
export class AnsiCodeTracker {
    ...
    getAnsiCode() {
        ...
        if (this.state.strikethrough)
            parts.push("9");
        if (this.state.fg)
            parts.push(this.state.fg);
        if (this.state.bg)
            parts.push(this.state.bg);
        if (parts.length === 0)
            return "\x1b[0m";
...
export function truncateToWidth(s, width, opts = {}) {
    if (width <= 0)
        return "";
...
// ─── Slice by Column ───────────────────────────────────────────────────────
export function sliceWithWidth(s, maxWidth) {
    let result = "";
    let width = 0;
    let i = 0;
...
export function sliceByColumn(s, start, end) {
    if (start < 0)
        start = 0;
    let col = 0;
    let i = 0;
