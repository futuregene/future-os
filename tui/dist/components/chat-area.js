...
export class ChatArea {
    ...
    lastRenderWidth = -1;
    dirty = false; // true when messages changed but render() not yet called
    md = new MarkdownRenderer();
    theme;
    constructor(maxWidth = 80, theme) {
    ...
    scrollDown(lines = 5) {
        this.viewportTop = Math.min(Math.max(0, this.renderedLines.length - this.viewportHeight), this.viewportTop + lines);
    }
    scrollToBottom() {
        this.viewportTop = Math.max(0, this.renderedLines.length - this.viewportHeight);
    }
    ...
    render(width) {
        ...
        const visible = this.renderedLines.slice(this.viewportTop, this.viewportTop + this.viewportHeight);
