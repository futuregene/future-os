//! Auth module - reads API credentials from ~/.future_agent/auth.json or ~/.future_agent-app/auth.json
...
impl AuthStore {
    ...
    pub fn load() -> Self {
        let paths = vec![
            dirs::home_dir()
                .map(|h| h.join(".future_agent-app/auth.json"))
                .unwrap_or_default(),
            dirs::home_dir()
                .map(|h| h.join(".future_agent/auth.json"))
                .unwrap_or_default(),
        ];
