//! future_agent — Rust agent backend (CLI entry point)

use anyhow::Result;
use clap::Parser;
use std::env;
use std::sync::Arc;
use future_agent_agent::{
    Engine, EngineConfig, Manager, ModelRegistry, ServerSession,
    USER_SKILLS_DIR, PROJECT_SKILLS_DIR, AGENTS_SKILLS_DIR,
};

#[derive(Parser)]
#[command(name = "future_agent")]
#[command(version = future_agent_agent::utils::VERSION)]
struct Cli {
...
async fn main() -> Result<()> {
    ...
    let cwd = if let Some(home) = dirs::home_dir() {
        ...
        if workspace.exists() {
            ...
            env::current_dir()
                .unwrap_or_else(|_| home.join("future_agent/agent"))
                .to_string_lossy()
                .to_string()
        }
    } else {
    ...
    let resolved_model = cli
        ...
        .or_else(|| future_agent_agent::models::get_default_model())
        .or_else(|| {
    ...
    let auth_store = future_agent_agent::AuthStore::load();
    ...
    let engine = Engine::new(&base_url, &api_key, &resolved_model, config)?
        .with_tools(future_agent_agent::all_tools());
    
    // Create event bus for server mode
    let event_bus = Arc::new(future_agent_agent::EventBus::new());
    ...
    if cli.messages.is_empty() {  // No messages → start server mode for TUI
        ...
        let skills = future_agent_agent::discover_skills(&skill_dirs).unwrap_or_default();
        let skill_names: Vec<String> = skills.iter().map(|s| s.name.clone()).collect();

        let manager = Arc::new(Manager::default_for(&cwd));
        let broadcaster: Arc<future_agent_agent::rpc::SseBroadcaster> = Arc::new(future_agent_agent::rpc::SseBroadcaster::new());
        let mut server_session = ServerSession::new(
            future_agent_agent::utils::generate_id(),
        ...
        let app_state = future_agent_agent::rpc::AppState {
            ...
            welcome_version: future_agent_agent::utils::VERSION.to_string(),
        ...
        future_agent_agent::grpc::serve(app_state, grpc_host, grpc_port).await?;
        return Ok(());
    }

    // Non-server mode: run prompt
    if cli.messages.is_empty() {
        eprintln!("Usage: future_agent [options] [messages...]");
        std::process::exit(1);
    }
    ...
    let messages = vec![future_agent_agent::AgentMessage::new_user(
        "user",
