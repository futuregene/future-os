/**
 * future_tui TUI - Main application.
 * Complete terminal UI for future_tui agent.
 */
...
export class App extends Container {
  ...
  private readonly slashCommands: SlashCommand[] = [
    ...
    { value: "/quit", label: "/quit", description: "quit future_tui" },
  ];
  ...
  private showWelcome(): void {
    ...
    // Banner: "future_tui vX.X.X"
    const version = this.state.version || "0.3.0";
    addColored(`${fg(151, bold("future_tui"))}${fg(245, " v" + version)}`, (t) => t);
  ...
  private renderHelp(W: number): string[] {
    ...
    lines.push(dim_("\u2502") + "  " + bold_("future_tui") + "  " + dim_("Terminal UI Help") + " ".repeat(Math.max(0, W - 24)) + dim_("\u2502"));
    lines.push(dim_("\u251c" + "\u2500".repeat(W - 2) + "\u2524"));
