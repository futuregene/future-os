/**
 * future_tui TypeScript TUI entry point.
 *
 * Usage:
...
// Handle --version
if (args.version) {
  console.log("future_tui TUI v0.3.0");
  process.exit(0);
}
...
if (args.print) {
  if (args.messages.length === 0 && args.fileArgs.length === 0) {
    if (args.mode !== "json") {
      console.error("No prompt provided. Usage: future_tui -p \"message\"");
    }
    process.exit(1);
  }
